...
1.2.4 -
   Fixes:
   * Updated scripts/Custom/Autoload/PATCH-SlipstreamModuleLoadFlash&Sound.py to 0.22, fixing one goof due to a type that made the patch break the install if no USS Sovereign's Slipstream mod was installed.
   * Updated scripts/Custom/TravellingMethods/Slipstream.py to 0.17, trying to make one re-skin function a bit more robust.
1.2.3 -
   Updates:
