...
def CreateSmokeHigh(sTexture, fFrequency, fVelocity, fSize, pEmitFrom, kEmitPos, kEmitDir, pAttachTo):
    ...
    pSmoke.AddColorKey(0.1, 1.0, 0.8, 0.1)
    pSmoke.AddColorKey(1.0, 0.996863, 0.904510, 0.137255)
    pSmoke.AddColorKey(1.0, 0.0, 0.0, 0.0)

    pSmoke.AddAlphaKey(0.0, 1.0)
    pSmoke.AddAlphaKey(0.7, 0.5)
    pSmoke.AddAlphaKey(1.0, 0.0)

    pSmoke.AddSizeKey(0.0, 0.016 * fSize)
    pSmoke.AddSizeKey(0.1, 0.23 * fSize)
    pSmoke.AddSizeKey(1.0, 0.16 * fSize)

    ## Setup properties
    pSmoke.SetEmitVelocity(fVelocity)
    pSmoke.SetAngleVariance(60.0)
    pSmoke.SetEmitLife(4.0) # TO-DO check these
    pSmoke.SetEmitFrequency(fFrequency)
    pSmoke.SetEffectLifeTime(12.0)
    pSmoke.SetDrawOldToNew(0)
    pSmoke.CreateTarget(sTexture)
    pSmoke.SetTargetAlphaBlendModes(0, 0)
...
def SetupSmokeTrail(pTorpedo):
    ...
    pSequence.AddAction(CreateSmokeHigh('scripts/Custom/NanoFXv2/SpecialFX/Gfx/Plasma/Plasma.tga', 0.001, 0.01, pTorpedo.GetRadius() * 3, pEmitFrom, App.NiPoint3(0, 0, 0), App.NiPoint3(0, 0, 0), pEffectRoot))
    pSequence.Play()
...
lLoadedTexturesCache = ['scripts/Custom/NanoFXv2/SpecialFX/Gfx/Plasma/Plasma.tga']
