...
def CreateAI(pShip):
        ...
        pAttack = AI.Compound.BorgAttack.CreateAI(pShip, pTargets, Difficulty = 1.0, MaxFiringRange = 100.0, FollowTargetThroughWarp = 1, ChooseSubsystemTargets = 1, AggressivePulseWeapons = 0, DisableBeforeDestroy = 0, InaccurateTorps = 1, SmartWeaponBalance = 0, SmartShields = 1, UseSideArcs = 1, HighPower = 1, PowerManagement = 1)
        # Done creating CompoundAI Attack
        #########################################

        # --- Add 5 second wait before engaging ---
        pTimePassed = App.ConditionScript_Create("Conditions.ConditionTimer", "ConditionTimer", 5, 0)

        pDelayedAttack = App.ConditionalAI_Create(pShip, "WaitToEngage")
        pDelayedAttack.SetInterruptable(1)
        pDelayedAttack.SetContainedAI(pAttack)
        pDelayedAttack.AddCondition(pTimePassed)

        def EvalWaitFunc(bTimePassed):
        ...
        pDelayedAttack.SetEvaluationFunction(EvalWaitFunc)

        #########################################
        # Creating ConditionalAI TargetInRange at (86, 150)
        ## Conditions:
        #### Condition InRange
        pInRange = App.ConditionScript_Create("Conditions.ConditionInRange", "ConditionInRange", 1000, pShip.GetName(), MissionLib.GetFriendlyGroup())
        ## Evaluation function:
        ...
        pTargetInRange.SetContainedAI(pDelayedAttack)  # now delayed
