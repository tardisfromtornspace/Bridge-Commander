...
# Config path
sPath = "scripts\\Custom\\UnifiedMainMenu\\ConfigModules\\Options\\SavedConfigs\\WarpspaceConfiguration.py"
...
# dicts
dGFXTunnel = {"Default" : "Default", "Alternate 1" : "WarpspaceTunnelAlt1.tga", "Alternate 2" : "WarpspaceTunnelAlt2.tga", "Alternate 3" : "WarpspaceTunnelAlt3.tga", "Alternate 4" : "WarpspaceTunnelAlt4.tga", "Alternate 5" : "WarpspaceTunnelAlt5.tga", "Alternate 6" : "WarpspaceTunnelAlt6.tga", "Alternate 7" : "WarpspaceTunnelAlt7.tga"}
dGFXTunnelTrans = {"Default" : "Default", "WarpspaceTunnelAlt1.tga" : "Alternate 1", "WarpspaceTunnelAlt2.tga": "Alternate 2", "WarpspaceTunnelAlt3.tga" : "Alternate 3", "WarpspaceTunnelAlt4.tga" : "Alternate 4", "WarpspaceTunnelAlt5.tga" : "Alternate 5", "WarpspaceTunnelAlt6.tga" : "Alternate 6", "WarpspaceTunnelAlt7.tga" : "Alternate 7"}
dFlash = {"Default" : "Default", "Alternate" : "WarpspaceFlashAlternate.tga"}
dFlashTrans = {"Default" : "Default", "WarpspaceFlashAlternate.tga" : "Alternate"}
dBackdrop = {"Default" : "Default", "Alternate" : "WarpspaceBackdropAlt.tga", "Off" : "Off"}
dBackdropTrans = {"Default" : "Default", "WarpspaceBackdropAlt.tga" : "Alternate", "Off" : "Off"}


# Returns a name to Unified Main Menu
def GetName():
        return "The Warpspace"
...
def CreateGFXOptions(pOptionPane, pContentPanel):
        ...
        pMainMenu = App.STCharacterMenu_Create("Warpspace GFX")
        ...
        for s in lGFXTunnel:
            if dGFXTunnelTrans[WarpspaceConfiguration.TunnelGFX] == s:
                pGFX.SetName(App.TGString("Tunnel GFX:  " + s))
                break

        for s in lFlash:
            if dFlashTrans[WarpspaceConfiguration.FlashGFX] == s:
                pFlash.SetName(App.TGString("Flash GFX:  " + s))
                break

        for s in lBackdrop:
            if dBackdropTrans[WarpspaceConfiguration.BackdropGFX] == s:
...
def CreateMiscOptions(pOptionPane, pContentPanel):
        ...
        pLoop = WarpspaceConfiguration.LoopSound
        ...
        pPrints = WarpspaceConfiguration.Prints
        ...
        pArrivingAt = WarpspaceConfiguration.ArrivingAt
...
def CreateAdditionalOptions(pOptionPane, pContentPanel):
        ...
        pButton = CreateButton("Warpspace Credits", pMainMenu, pOptionPane, pContentPanel, __name__ + ".ModCredits", EventInt = 0)
...
def PreviewAllGFX(pObject, pEvent):
        ...
        from Custom.Warpspace.Libs import PreviewVid
...
def ModCredits(pObject, pEvent):
        from Custom.Warpspace.Libs import Credits
...
def SaveConfiguration(strGFX = None, strFlash = None, strBackdrop = None):
        ...
        if not strGFX == None:
            ...
            sGFX = WarpspaceConfiguration.TunnelGFX
        if not strFlash == None:
            sFlash = dFlash[strFlash]
        else:
            sFlash = WarpspaceConfiguration.FlashGFX
        if not strBackdrop == None:
            sBackdrop = dBackdrop[strBackdrop]
        else:
            sBackdrop = WarpspaceConfiguration.BackdropGFX
