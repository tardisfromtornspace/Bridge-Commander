...
# Config path
sPath = "scripts\\Custom\\UnifiedMainMenu\\ConfigModules\\Options\\SavedConfigs\\TimeVortexConfiguration.py"
...
# dicts
dGFXTunnel = {"Default" : "Default", "Alternate 1" : "TimeVortexTunnelAlt1.tga", "Alternate 2" : "TimeVortexTunnelAlt2.tga", "Alternate 3" : "TimeVortexTunnelAlt3.tga", "Alternate 4" : "TimeVortexTunnelAlt4.tga", "Alternate 5" : "TimeVortexTunnelAlt5.tga", "Alternate 6" : "TimeVortexTunnelAlt6.tga", "Alternate 7" : "TimeVortexTunnelAlt7.tga"}
dGFXTunnelTrans = {"Default" : "Default", "TimeVortexTunnelAlt1.tga" : "Alternate 1", "TimeVortexTunnelAlt2.tga": "Alternate 2", "TimeVortexTunnelAlt3.tga" : "Alternate 3", "TimeVortexTunnelAlt4.tga" : "Alternate 4", "TimeVortexTunnelAlt5.tga" : "Alternate 5", "TimeVortexTunnelAlt6.tga" : "Alternate 6", "TimeVortexTunnelAlt7.tga" : "Alternate 7"}
dFlash = {"Default" : "Default", "Alternate" : "TimeVortexFlashAlternate.tga"}
dFlashTrans = {"Default" : "Default", "TimeVortexFlashAlternate.tga" : "Alternate"}
dBackdrop = {"Default" : "Default", "Alternate" : "TimeVortexBackdropAlt.tga", "Off" : "Off"}
dBackdropTrans = {"Default" : "Default", "TimeVortexBackdropAlt.tga" : "Alternate", "Off" : "Off"}


# Returns a name to Unified Main Menu
def GetName():
        return "The TimeVortex"
...
def CreateGFXOptions(pOptionPane, pContentPanel):
        ...
        pMainMenu = App.STCharacterMenu_Create("TimeVortex GFX")
        ...
        for s in lGFXTunnel:
            if dGFXTunnelTrans[TimeVortexConfiguration.TunnelGFX] == s:
                pGFX.SetName(App.TGString("Tunnel GFX:  " + s))
                break

        for s in lFlash:
            if dFlashTrans[TimeVortexConfiguration.FlashGFX] == s:
                pFlash.SetName(App.TGString("Flash GFX:  " + s))
                break

        for s in lBackdrop:
            if dBackdropTrans[TimeVortexConfiguration.BackdropGFX] == s:
...
def CreateMiscOptions(pOptionPane, pContentPanel):
        ...
        pLoop = TimeVortexConfiguration.LoopSound
        ...
        pPrints = TimeVortexConfiguration.Prints
        ...
        pArrivingAt = TimeVortexConfiguration.ArrivingAt
...
def CreateAdditionalOptions(pOptionPane, pContentPanel):
        ...
        pButton = CreateButton("TimeVortex Credits", pMainMenu, pOptionPane, pContentPanel, __name__ + ".ModCredits", EventInt = 0)
...
def PreviewAllGFX(pObject, pEvent):
        ...
        from Custom.TimeVortex.Libs import PreviewVid
...
def ModCredits(pObject, pEvent):
        from Custom.TimeVortex.Libs import Credits
...
def SaveConfiguration(strGFX = None, strFlash = None, strBackdrop = None):
        ...
        if not strGFX == None:
            ...
            sGFX = TimeVortexConfiguration.TunnelGFX
        if not strFlash == None:
            sFlash = dFlash[strFlash]
        else:
            sFlash = TimeVortexConfiguration.FlashGFX
        if not strBackdrop == None:
            sBackdrop = dBackdrop[strBackdrop]
        else:
            sBackdrop = TimeVortexConfiguration.BackdropGFX
