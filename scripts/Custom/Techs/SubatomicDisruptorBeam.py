...
#         This tech depends on FIX-FoundationTech20050703DefendVSTorpFix by Alex SL Gato to patch FoundationTech properly.
#################################################################################################################
#
...
#
# Start on 2:
# This tech makes ships gain Subatomic Beam tech, which will 1. makes beams be able to deal damage through shields if configured corectly and 2. makes them deal the same damage regardless of distance. It still makes beam yields variable to 3 damage-dealing status with the phaser level slider or button: full-power (100%), half-power (>= 50%) and no damage (<50%) (useful for manually tractoring targets or when on green or yellow alert so a vessel does not harm another).
# To add it, just add to your Custom/Ships/shipFileName.py this:
"""
Foundation.ShipDef.Ambassador.dTechs = {
	"Subatomic Disruptor Beams Weapon": [{"HullDmgMultiplier": 100.0, "ShieldDmgMultiplier": 1.0, "Beams": ["Tractor beam projector name 1", "Tractor beam projector name 2"]}, ["Beam Sound 1 to add", "Beam 2 sound to add"]],
}
"""
...
# "Beams" is an optional list with Phaser Bank names, wich will narrow the tractor beam projectors to a select few. Not adding the field or making it blank will mean all tractor beams are Subatomic beam yields... if they have the proper sound.
#
...
##
# If you want an new specific subTech that modifies part of Subatomic Beams Effect, you can do it by adding a file under the scripts\Custom\Techs\SubatomicBeamWeaponScripts directory; if possible with a reasonable name related to the Technology(ies) it covers.
...
# Version: 1.0
# Meant to be used alongside the SubatomicRepulsorTech Technology (located at scripts/Custom/Techs), this file must be under scripts/Custom/Techs/SubatomicBeamWeaponScripts
# As these are Sub-techs with some leeway, their manuals must be explained here:
##################################
# SPECIFIC SUB-TECH MANUAL:
# This file takes care of how SG shielding is affected by Subatomic Beam Weapons.
...
SubatomicBeamsGenericShieldDamageMultiplier = 1.0
SubatomicBeamsHullDamageMultiplier = 1.0

# At the moment I cannot really think of one ship immune to both effects in particular legacy-wise... adding this Dummy
global lImmuneSubatomicBeamsWeaponShips
lImmuneSubatomicBeamsWeaponShips = []
...
# "variableNames" is a global dictionary, filled automatically by the files in scripts/Custom/Techs/SubatomicBeamsWeaponScripts 
...
# Alongside these parameters, there are additional ones, but will be ignored unless they are on the "BasicSubatomicBeamsWeaponConfiguration.py" subTech file:
## "SubatomicBeamsHullDamageMultiplier": allows to edit the base damage multiplier to all hulls without needing to edit the main technology file. Default is 1 (1x Equal damage).
## "SubatomicBeamsGenericShieldDamageMultiplier": allows to edit the base damage multiplier to all shields without needing to edit the main technology file. Default is 1 (deal damage x1)
...
def LoadExtraLimitedPlugins(dExcludePlugins=_g_dExcludeSomePlugins):
	...
	dir="scripts\\Custom\\Techs\\SubatomicDisruptorBeamWeaponScripts" # I want to limit any vulnerability as much as I can while keeping functionality
	import string

	try:
		list = nt.listdir(dir)
		if not list:
			print "ERROR: Missing scripts/Custom/Techs/SubatomicDisruptorBeamWeaponScripts folder for SubatomicBeamWeapon technology"
			return 0

	except:
		print "ERROR: Missing scripts/Custom/Techs/SubatomicDisruptorBeamWeaponScripts folder for SubatomicBeamWeapon technology, or other error:"
		traceback.print_exc()
		return 0
	list.sort()
			...
			try:
				if not variableNames.has_key(fileName):
					...
					try:
						...
						try:
							banana = __import__(myGoodPlugin, globals(), locals(), ["SubatomicBeamsHullDamageMultiplier", "SubatomicBeamsGenericShieldDamageMultiplier", "SlowDownRatio"])
						except:
					...
					if hasattr(banana, "legacyImmunity"): # These will not be affected at all by the hull or shield effects, period
						...
						for item in banana.legacyImmunity:
							lImmuneSubatomicBeamsWeaponShips.append(item)
					...
					if fileName == "SubatomicBeamWeaponBasicConfiguration":
						if hasattr(banana, "SubatomicBeamsHullDamageMultiplier"):
							global SubatomicBeamsHullDamageMultiplier
							SubatomicBeamsHullDamageMultiplier = banana.SubatomicBeamsHullDamageMultiplier

						if hasattr(banana, "SubatomicBeamsGenericShieldDamageMultiplier"):
							global SubatomicBeamsGenericShieldDamageMultiplier
							SubatomicBeamsGenericShieldDamageMultiplier = banana.SubatomicBeamsGenericShieldDamageMultiplier

						if hasattr(banana, "SlowDownRatio"):
							global SlowDownRatio
							SlowDownRatio = banana.SlowDownRatio

					#print "SubatomicBeamsWeapon reviewing of this subtech is a success"
			except:
				print "someone attempted to add more than they should to the SubatomicRepulsorTech script"
				traceback.print_exc()
...
#print SubatomicBeamsHullDamageMultiplier, SubatomicBeamsGenericShieldDamageMultiplier
#print variableNames
...
try:
	...
	import Appc
	class FakeSubatomicDisruptorWeaponHitEvent(App.TGEvent):
		...
		def __repr__(self):
			return "<Python FakeSubatomicDisruptorWeaponHitEvent instance at %s>" % (self.this,)

	class FakeSubatomicDisruptorWeaponHitEventPtr(FakeSubatomicDisruptorWeaponHitEvent):
		def __init__(self,this):
			self.this = this
			self.thisown = 0
			self.__class__ = FakeSubatomicDisruptorWeaponHitEvent

	def TGFakeSubatomicDisruptorWeaponHitEvent_Create(*args, **kwargs):
		val = apply(Appc.TGEvent_Create,args,kwargs)
		if val:
			val = FakeSubatomicDisruptorWeaponHitEventPtr(val)
		return val

	class SubatomicBeamsWeapon(FoundationTech.TechDef):
			...
			if pInstance != None:
				...
				if pInstance.__dict__.has_key(self.name):
					...
					if str(pConfig)[0] != "[":
						...
						if pShip:
							pTractor = pShip.GetPhaserSystem()
							if not pTractor:
								return
							pProjector = App.PhaserBank_Cast(pTractor.GetChildSubsystem(0))
							if not pProjector:
								return
		...
		def IsSubatomicBeamsWeaponYield(self):
			debug(__name__ + ", IsSubatomicBeamsWeaponYield")
			return 1
		...
		def GetMySupportProjectile(self):
			return "Tactical.Projectiles.SubatomicBeamDummy"
		...
		def OneWeaponHit(self, pEvent, pPTarget = None, pTargetInstanceAux = None, pTorp = None, time = None):
			...
			try:
				...
				if hasattr(pEvent, "GetWeaponType") and pEvent.GetWeaponType() != pEvent.PHASER:
					return 0
				...
				if hasattr(pEvent, "GetFiringObject"): # If we use App.ET_WEAPON_HIT, it is this
					...
					evtHType = 1
					pWeaponFired = App.PhaserBank_Cast(pEvent.GetSource())
				...
				global lImmuneSubatomicBeamsWeaponShips # This legacy list goes first - it is to reduce unnecessary calculations
				if sShipScript in lImmuneSubatomicBeamsWeaponShips:
					return 0
				...
				if pTargetInstance:
					...
					if pTargetInstanceDict.has_key('Subatomic Disruptor Beams Weapon Immune'):
						immuTyp = pTargetInstanceDict['Subatomic Disruptor Beams Weapon Immune']
						if(pEventIsHullHit):
				...
				if cfg0dict.has_key("Beams") and len(cfg0dict["Beams"]) > 0:
					...
					if not pWeaponFired.GetName() in lBeamNames:
						#print "SubatomicBeamWeapon: cancelling, ship has SubatomicBeamWeapon equipped but not for that beam..."
						return
				#else:
				#	print __name__, ": I do not have beams key, I will assume all beams have ", self.name, " ability"
				...
				myHull = None
				myHullPos = None
				global SubatomicBeamsGenericShieldDamageMultiplier, SubatomicBeamsHullDamageMultiplier

				if evtHType == 1:
					if fRadius == None:
						fRadius = targetRadius
					if fDamage == None:
						fDamage = SubatomicBeamsHullDamageMultiplier
				...
				baseHullMultiplier = 1.0 * SubatomicBeamsHullDamageMultiplier
				baseShieldMultiplier = 1.0 * SubatomicBeamsGenericShieldDamageMultiplier
				...
				try:
					...
					if targetRadius < 0.15 or evtHType == 1:
						...
						if myHull:
							...
							pEvent1 = TGFakeSubatomicDisruptorWeaponHitEvent_Create() # This gives us a pointer, it has no arguments, but it is our "fake" event
	...
	oSubatomicBeamsWeapon = SubatomicBeamsWeapon("Subatomic Disruptor Beams Weapon")
...
class SubatomicBeamsWeaponDef(FoundationTech.TechDef):
	def OnTractorDefense(self, pShip, pInstance, oYield, pEvent):
		return self.CommonCheck(pShip, pInstance, None, oYield, pEvent)

	def OnBeamDefense(self, pShip, pInstance, oYield, pEvent):
		return self.CommonCheck(pShip, pInstance, None, oYield, pEvent)

	def OnPulseDefense(self, pShip, pInstance, pTorp, oYield, pEvent):
		return self.CommonCheck(pShip, pInstance, pTorp, oYield, pEvent)

	def OnTorpDefense(self, pShip, pInstance, pTorp, oYield, pEvent):
		return self.CommonCheck(pShip, pInstance, pTorp, oYield, pEvent)

	def CommonCheck(self, pShip, pInstance, pTorp, oYield, pEvent):
		debug(__name__ + ", CommonCheck")
		if oYield and hasattr(oYield, "IsSubatomicBeamsWeaponYield") and oYield.IsSubatomicBeamsWeaponYield() != 0 and pInstance and pInstance.__dict__.has_key(self.name) and pInstance.__dict__[self.name] >= 2:
			return 1
...
oSubatomicBeamsWeaponImmunity = SubatomicBeamsWeaponDef('Subatomic Disruptor Beams Weapon Immune')