...
# This file adds Compressors to the list of guaranteed bleedthrough to ME Shields (according to the info aout particle beam weapons from collectors "Being a particle beam weapon, it ignores kinetic barriers,"), according to MEShields in-script documentation. Due to the parent script implementation, we could add multiple files that add elements, and repeated elements on the lists will not be repeatedly included on the parent script.
...
BeamWhitelistHardname = [ ]
BeamBlacklistName = ["gravity", " particleless", " not compressor", " no compressor", "mass effect",]
