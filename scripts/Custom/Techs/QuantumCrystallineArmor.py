...
class AblativeDef(FoundationTech.GaugeTechDef):
	...
	def OnDefense(self, pShip, pInstance, oYield, pEvent):
		repair = pInstance.__dict__['Quantum Crystalline Armor']
		...
		if repair > 0.0:
			...
			if str(pInstance.__dict__['Quantum Crystalline Armor'])[0] == "[":
				pInstance.__dict__['Quantum Crystalline Armor'][0] = repair = repair - pEvent.GetDamage()
				
			else:
				pInstance.__dict__['Quantum Crystalline Armor'] = repair = repair - pEvent.GetDamage()
			...
			while pSubsystem:
				if pSubsystem.GetName() == pSubName:
					...
					# pSubsystem.SetCondition(pInstance.__dict__['Quantum Crystalline Armor'])
...
oAblative = AblativeDef('Quantum Crystalline Armor')
...
# 	#		if pInstance.__dict__.has_key("Quantum Crystalline Armor"):
# 	 	pIterator = pShip.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
# 		pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
# 		while pSubsystem != None:
# 			if pSubsystem.GetName() == "Quantum Crystalline Armor":
# 				pArmour = pSubsystem
# 				pShip.EndGetSubsystemMatch(pIterator)
# 				break
# 			pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
...
# #Foundation.OverrideDef.FTBQuantumCrystallineArmorGUI = Foundation.OverrideDef("FTB Quantum Crystalline Armor GUI", "Tactical.Interface.ShipDisplay.SetShipID", __name__ ".SetShipID", FoundationTech.dMode)
# 
