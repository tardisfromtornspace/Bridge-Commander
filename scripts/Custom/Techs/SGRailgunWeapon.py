...
#
# Start on 2:
# This tech is meant to adjust some SG Tau'ri Railgun Weapons to behave a bit more like in the Stargate show, alongside its own sub-technologies that makes the behaviour even more canonical.
# On this case, the tech is here to adjust to the fact these weapons are super-effective against unshielded vessels but deal little damage to shielded vessels, at least those with SG shields.
# Additionally, it is added as a way to make another difference between SG Shields/Hulls and ST Shields/Hulls regarding Railgun weapons:
## If the Railgun weapon is small and/or primitive, ST shields block them extremely easily with no strain at all. However, if the weapon is big or extremely advanced, the shields may strain.
## On unshielded vessels, even primitive Railgun weapons can be problematic to handle.
...
try:
	modSGRailgunWeaponTorp = __import__("Custom.Techs.SGRailgunWeapon")
	if(modSGRailgunWeaponTorp):
		modSGRailgunWeaponTorp.oSGRailgunWeaponTorp.AddTorpedo(__name__)
except:
	print "SGRailgunWeapon projectile script not installed, or you are missing Foundation Tech"
"""
...
## "wasChanged": if this value is lesser than 0, it will perform the default effect (shield drain where "shouldDealAllFacetDamage = 0" and "shieldDamageMultiplier = RailgunGenericShieldDamageMultiplier" * maybe ShieldDmgMultiplier/"hullDamageMultiplier = pTorp.HullDmgMultiplier() - 1" or "hullDamageMultiplier = RailgunHullDamageMultiplier -1"). When some script changes things it is recommended to stack "1" to this value, unless you want a default behaviour with modified "shouldPassThrough" and "considerPiercing".
# If you want an new specific subTech that modifies part of the SG Railgun Effect, you can do it by adding a file under the scripts\Custom\Techs\SGRailgunWeaponScripts directory; if possible with a reasonable name related to the Technology(ies) it covers. 
...
# Below there's an example for a hypothethical SGShieldsConfiguration, at least the 1.0 version, but modified to include more function examples, clarify and with some parts commented so as to not trigger commentary issues - those sections have replaced the triple " with ####@@@
"""
# THIS FILE IS NOT SUPPORTED BY ACTIVISION
# THIS FILE IS UNDER THE LGPL FOUNDATION LICENSE AS WELL
...
# This file takes care of how SG shielding is affected by SG Railgun Weapons.
...
# On this case, the tech is here to adjust to the fact SG Railgun weapons are super-effective against unshielded vessels but deal little damage to shielded vessels, at least those with SG shields.
...
#RailgunHullDamageMultiplier = 4
#RailgunGenericShieldDamageMultiplier = 4
#SlowDownRatio = 3.0/75.0

RailgunSGVulnerableShields = [] # No actual SG vessel is vulnerable to Tau'ri Railgun cannons in the way the Tau'ri were to Tollan or Asgard Ion ones. Just leaving this here in case someone randomly wants to add that for a fanmade ship or something
RailgunSGShieldDamageMultiplier = 4.0
...
## Adding Asgard and Ori here, because the Asgard used neutronium on their vessel alloys, and for most of them the Railgun cannons were primitive enough that Asgard shields could handle them easier (excluding Anubis-grade ones)
...
xAnubisShieldMultiplier = 0.6 # Seems fair that when Anubis upgraded his shields, it also made them more resistant to the mainstream weapons the Tau'ri use
	...
	if pInstance and pInstancedict.has_key("SG Shields"):
		...
		if pInstancedict["SG Shields"].has_key("RaceShieldTech"):
			...
			if RaceShieldTech in RailgunSGVulnerableShields:
				shouldPassThrough = 1
			elif RaceShieldTech == "Anubis Tau'ri": # Resistances	
...
# "variableNames" is a global dictionary, filled automatically by the files in scripts/Custom/Techs/SGRailgunWeaponScripts 
...
# Alongside these parameters, there are additional ones, but will be ignored unless they are on the "BasicSGRailgunWeaponConfiguration.py" subTech file:
## "RailgunHullDamageMultiplier": allows to edit the base damage multiplier to all hulls without needing to edit the main technology file. Default is 10 (10 -1, to deal 10 times the original damage, this script deals those 9 extra).
## "RailgunGenericShieldDamageMultiplier": allows to edit the base damage multiplier to all shields without needing to edit the main technology file. Default is 2 (2 -1, means deal damage once more)
...
def LoadExtraLimitedPlugins(dExcludePlugins=_g_dExcludeSomePlugins):
	...
	dir="scripts\\Custom\\Techs\\SGRailgunWeaponScripts" # I want to limit any vulnerability as much as I can while keeping functionality
	import string
			...
			try:
				if not filesChecked.has_key(fileName):
					...
					try:
						...
						try:
							banana = __import__(myGoodPlugin, globals(), locals(), ["RailgunHullDamageMultiplier", "RailgunGenericShieldDamageMultiplier", "SlowDownRatio"])
						except:
					...
					if hasattr(banana, "legacyImmunity"): # These will not be affected at all by the hull or shield effects, period
						...
						for item in banana.legacyImmunity:
							lImmuneSGRailgunWeaponShips.append(item)
					...
					if fileName == "SGRailgunWeaponBasicConfiguration":
						if hasattr(banana, "RailgunHullDamageMultiplier"):
							global RailgunHullDamageMultiplier
							RailgunHullDamageMultiplier = banana.RailgunHullDamageMultiplier

						if hasattr(banana, "RailgunGenericShieldDamageMultiplier"):
							global RailgunGenericShieldDamageMultiplier
							RailgunGenericShieldDamageMultiplier = banana.RailgunGenericShieldDamageMultiplier

						if hasattr(banana, "SlowDownRatio"):
							global SlowDownRatio
							SlowDownRatio = banana.SlowDownRatio

					#print "SGRailgunWeapon reviewing of this subtech is a success"
			except:
				print "someone attempted to add more than they should to the SGRailgunWeapon script"
				traceback.print_exc()
...
#print RailgunHullDamageMultiplier, RailgunGenericShieldDamageMultiplier
#print variableNames
...
try:
	...
	class SGRailgunWeaponTorpedo(FoundationTech.TechDef):
		...
		def IsSGRailgunWeaponYield(self):
			return 1
		...
		def OnYield(self, pShip, pInstance, pEvent, pTorp):
			...
			global lImmuneSGRailgunWeaponShips # This legacy list goes first - it is to reduce unnecessary calculations
			if sShipScript in lImmuneSGRailgunWeaponShips:
				return
			...
			shieldDamageMultiplier = RailgunGenericShieldDamageMultiplier
			...
			if(pEvent.IsHullHit()):
				if pInstancedict.has_key('SG Railgun Weapon Immune') and (pInstancedict['SG Railgun Weapon Immune'] == 0 or pInstancedict['SG Railgun Weapon Immune'] > 1):
					return
				wasHullChanged = 0
				for item in variableNames.keys():
					if variableNames[item].has_key("interactionHullBehaviour"): # These are reserved for when the hull has been hit! These are meant to be accumulative, for defenses.
						...
						try:
							...
							print "Some SGRailgunWeapon hull subtech suffered an error"
							traceback.print_exc()
				...
				global SlowDownRatio
				mod = "Tactical.Projectiles.ExtraPhasedDamageDummyRailgun" # This torpedo was made so Automated Point Defence scripts stop harrasing us! 
				try:
					...
					print "You are missing 'Tactical.Projectiles.ExtraPhasedDamageDummy' torpedo on your install, without that the SG Railgun Weapons here cannot deal extra hull damage... or another error happened"
					traceback.print_exc()
			...
			if pInstancedict.has_key('SG Railgun Weapon Immune') and pInstancedict['SG Railgun Weapon Immune'] > 0:
				return
			...
			for item in variableNames.keys():
				if variableNames[item].has_key("interactionShieldBehaviour"): # These are reserved for when the shield has been hit! Also be careful, since these are more likely to stack weaknesses! Preferable to only have one of this type per ship
					try:
						...
						print "Some SGRailgunWeapon shield subtech suffered an error"
						traceback.print_exc()
			...
			if wasShieldChanged <= 0:
				...
				shieldDamageMultiplier = RailgunGenericShieldDamageMultiplier - 1 # nullify shield effects - that is, is immune
				mod = pTorp.GetModuleName()
				importedTorpInfo = __import__(mod)
				if hasattr(importedTorpInfo, "ShieldDmgMultiplier"): # If this torp has a special global multiplier, then we use it
					shieldDamageMultiplier = shieldDamageMultiplier * importedTorpInfo.ShieldDmgMultiplier()
			else:
				shieldDamageMultiplier = shieldDamageMultiplier - RailgunGenericShieldDamageMultiplier # That is, to compensate the "extra" time we added 1 damage
	...
	oSGRailgunWeaponTorp = SGRailgunWeaponTorpedo("SGRailgunWeapon Torpedo")
	# Just a few standard torps I know of that are SGRailgunWeapon... 
...
class SGRailgunWeaponDef(FoundationTech.TechDef):
	...
	def OnProjectileDefense(self, pShip, pInstance, pTorp, oYield, pEvent):
		if oYield and hasattr(oYield, "IsSGRailgunWeaponYield") and oYield.IsSGRailgunWeaponYield() != 0 and pInstance and pInstance.__dict__.has_key('SG Railgun Weapon Immune') and pInstance.__dict__['SG Railgun Weapon Immune'] >= 2:
			return 1
...
oSGRailgunWeaponImmunity = SGRailgunWeaponDef('SG Railgun Weapon Immune')