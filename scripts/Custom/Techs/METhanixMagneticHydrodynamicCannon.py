...
#         Based on SGAsgardBeamWeapon.py, which was based on BorgAdaptation.py and PhasedTorp.py by Alex SL Gato, which were based on the Foundation import function by Dasher; the Shield.py scripts and KM Armour scripts and FoundationTechnologies team's PhasedTorp.py
...
Foundation.ShipDef.Ambassador.dTechs = {
	"ME Thanix Magnetic-Hydrodynamic Cannon": {"HullDmgMultiplier": 1.0, "ShieldDmgMultiplier": 1.0, "DShieldDmgMultiplier": 1.0, "Beams": ["Beam name 1", "Beam name 2"]},
}
"""
...
# "DShieldDmgMultiplier" will multiply upon the base global multiplier for shield damage if no shield changes were done. Default is x1.0.
# "Beams" is an optional list with Phaser Bank names, wich will narrow the beams to a select few. Not adding the field or making it blank will mean all beams are Thanix Magnetic-Hydrodynamic Cannons.
...
# Below there's an example used for the aforementioned MEShieldsConfiguration, at least the 1.0 version, but modified to include more function examples, clarify and with some parts commented so as to not trigger commentary issues - those sections have replaced the triple " with ####@@@
"""
# THIS FILE IS NOT SUPPORTED BY ACTIVISION
# THIS FILE IS UNDER THE LGPL FOUNDATION LICENSE AS WELL
# 19th December 2024, by Alex SL Gato (CharaToLoki)
...
METhanixGenericShieldDamageMultiplier = 100.0
METhanixDefaultShieldMultiplier = 0.01
METhanixHullDamageMultiplier = 50.0

# At the moment I cannot really think of one ship immune to both effects in particular legacy-wise... adding this Dummy
global lImmuneMEThanixShips
lImmuneMEThanixShips = []
...
# "variableNames" is a global dictionary, filled automatically by the files in scripts/Custom/Techs/METhanixScripts 
...
# Alongside these parameters, there are additional ones, but will be ignored unless they are on the "BasicMEThanixConfiguration.py" subTech file:
## "METhanixHullDamageMultiplier": allows to edit the base damage multiplier to all hulls without needing to edit the main technology file. Default is 10 (10 -1, to deal 10 times the original damage, this script deals those 9 extra).
## "METhanixGenericShieldDamageMultiplier": allows to edit the base damage multiplier to all shields without needing to edit the main technology file. Default is 2 (2 -1, means deal damage once more)
...
def LoadExtraLimitedPlugins(dExcludePlugins=_g_dExcludeSomePlugins):
	...
	dir="scripts\\Custom\\Techs\\METhanixScripts" # I want to limit any vulnerability as much as I can while keeping functionality
	import string

	try:
		list = nt.listdir(dir)
		if not list:
			print "ERROR: Missing scripts/Custom/Techs/METhanixScripts folder for METhanixMagneticHydrodynamicCannon technology"
			return 0

	except:
		print "ERROR: Missing scripts/Custom/Techs/METhanixScripts folder for METhanixMagneticHydrodynamicCannon technology, or other error:"
		traceback.print_exc()
		return 0
	list.sort()
			...
			try:
				if not variableNames.has_key(fileName):
					...
					try:
						...
						try:
							banana = __import__(myGoodPlugin, globals(), locals(), ["METhanixHullDamageMultiplier", "METhanixGenericShieldDamageMultiplier", "SlowDownRatio"])
						except:
					...
					if hasattr(banana, "legacyImmunity"): # These will not be affected at all by the hull or shield effects, period
						...
						for item in banana.legacyImmunity:
							lImmuneMEThanixShips.append(item)
					...
					if fileName == "METhanixBasicConfiguration":
						if hasattr(banana, "METhanixHullDamageMultiplier"):
							global METhanixHullDamageMultiplier
							METhanixHullDamageMultiplier = banana.METhanixHullDamageMultiplier

						if hasattr(banana, "METhanixGenericShieldDamageMultiplier"):
							global METhanixGenericShieldDamageMultiplier
							METhanixGenericShieldDamageMultiplier = banana.METhanixGenericShieldDamageMultiplier

						if hasattr(banana, "METhanixDefaultShieldMultiplier"):
							global METhanixDefaultShieldMultiplier
							METhanixDefaultShieldMultiplier = banana.METhanixDefaultShieldMultiplier

						if hasattr(banana, "SlowDownRatio"):
							global SlowDownRatio
							SlowDownRatio = banana.SlowDownRatio

					#print "METhanixMagneticHydrodynamicCannon reviewing of this subtech is a success"
			except:
				print "someone attempted to add more than they should to the METhanixMagneticHydrodynamicCannon script"
				traceback.print_exc()
...
#print METhanixHullDamageMultiplier, METhanixGenericShieldDamageMultiplier
#print variableNames
...
try:
	...
	class METhanixWeapon(FoundationTech.TechDef):
		...
		def IsMEThanixYield(self):
			debug(__name__ + ", IsMEThanixYield")
			return 1

		def IsMEThanixMagneticHydrodynamicCannonYield(self):
			debug(__name__ + ", IsMEThanixMagneticHydrodynamicCannonYield")
			return 1
		...
		def shieldIsLesserThan(self, pShip, kPoint, extraDamageHeal, shieldThreshold = 0.2, multifacet = 0, negateRegeneration=0):
			...
			return shieldHitBroken, resultHeal
		...
		def OneWeaponHit(self, pEvent):
			...
			try:
				...
				if not pAttackerInstance or not pAttackerInstance.__dict__.has_key("ME Thanix Magnetic-Hydrodynamic Cannon"):
					return 0
				...
				global lImmuneMEThanixShips # This legacy list goes first - it is to reduce unnecessary calculations
				if sShipScript in lImmuneMEThanixShips:
					return 0
				...
				if pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"].has_key("Beams") and len(pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["Beams"]) > 0:
					#print "METhanixMagneticHydrodynamicCannon: I have beams key, verifying the phaser bank is among them"
					lBeamNames = pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["Beams"]		

					if not pWeaponFired.GetName() in lBeamNames:
						#print "METhanixMagneticHydrodynamicCannon: cancelling, ship has METhanixMagneticHydrodynamicCannon equipped but not for that beam..."
						return
				#else:
				#	print "METhanixMagneticHydrodynamicCannon: I do not have beams key, I will assume all phasers have ME Thanix weapons ability"
				...
				global METhanixGenericShieldDamageMultiplier, METhanixDefaultShieldMultiplier, METhanixHullDamageMultiplier

				baseHullMultiplier = 1.0 * METhanixHullDamageMultiplier
				baseShieldMultiplier = 1.0 * METhanixGenericShieldDamageMultiplier
				baseShieldDMultiplier = 1.0 * METhanixDefaultShieldMultiplier

				if pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"].has_key("HullDmgMultiplier") and pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["HullDmgMultiplier"] > 0.0:
					baseHullMultiplier = baseHullMultiplier * pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["HullDmgMultiplier"]

				if pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"].has_key("ShieldDmgMultiplier") and pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["ShieldDmgMultiplier"] > 0.0:
					baseShieldMultiplier = baseShieldMultiplier * pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["ShieldDmgMultiplier"]

				if pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"].has_key("DShieldDmgMultiplier") and pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["DShieldDmgMultiplier"] >= 0.0:
					baseShieldDMultiplier = baseShieldDMultiplier * pAttackerInstanceDict["ME Thanix Magnetic-Hydrodynamic Cannon"]["DShieldDmgMultiplier"]
				...
				if wasShieldChanged <= 0:
					# normal shields
					shouldPassThrough = 0
					baseShieldMultiplier = shieldDamageMultiplier * baseShieldDMultiplier
				else:
				...
				mod = "Tactical.Projectiles.METhanixCannonDummy" # This torpedo was made so Automated Point Defence scripts stop harrasing us
				...
				try:
					...
					print "Tactical.Projectiles.METhanixCannonDummy is missing from the install, or similar, we need that to deal damage!"
					traceback.print_exc()
					return 0
				...
				try:
					...
					if pEvent.IsHullHit() or shouldPassThrough > 0 or shouldDoHull > 0:
						if shouldPassThrough > 0 and shouldDoHull <= 0:
							finalTorpDamage = torpVSHullDamage
						elif not pEvent.IsHullHit() and shouldDoHull > 0:
							finalTorpDamage = currentShieldlvl + torpVSHullDamage
						else:
							finalTorpDamage = torpVSHullDamage
						leNetType = Multiplayer.SpeciesToTorp.PHASEDPLASMA
					else:
					...
					print "You are missing 'Tactical.Projectiles.METhanixCannonDummy' torpedo on your install, without that the ME Thanix Magnetic-Hydrodynamic Cannon here cannot deal extra hull damage... or another error happened"
					traceback.print_exc()		
			
			except:
				print "	Error when handling ME Thanix Magnetic-Hydrodynamic Cannon Hit"
				traceback.print_exc()
			return 0

	oMEThanix = METhanixWeapon("ME Thanix Magnetic-Hydrodynamic Cannon")
