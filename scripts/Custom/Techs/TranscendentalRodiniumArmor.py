...
class AblativeDef(FoundationTech.GaugeTechDef):
	...
	def OnDefense(self, pShip, pInstance, oYield, pEvent):
		repair = pInstance.__dict__['Transcendental Rodinium Armor']
		...
		if repair > 0.0:
			...
			if str(pInstance.__dict__['Transcendental Rodinium Armor'])[0] == "[":
				pInstance.__dict__['Transcendental Rodinium Armor'][0] = repair = repair - pEvent.GetDamage()
				
			else:
				pInstance.__dict__['Transcendental Rodinium Armor'] = repair = repair - pEvent.GetDamage()
			...
			while pSubsystem:
				if pSubsystem.GetName() == pSubName:
					...
					# pSubsystem.SetCondition(pInstance.__dict__['Transcendental Rodinium Armor'])
	...
	def GetSystemName(self):
		return "Transcendental Rodinium Armor"
...
oAblative = AblativeDef('Transcendental Rodinium Armor')
...
# 	#		if pInstance.__dict__.has_key("Transcendental Rodinium Armor"):
# 	 	pIterator = pShip.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
# 		pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
# 		while pSubsystem != None:
# 			if pSubsystem.GetName() == "Transcendental Rodinium Armor":
# 				pArmour = pSubsystem
# 				pShip.EndGetSubsystemMatch(pIterator)
# 				break
# 			pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
...
# #Foundation.OverrideDef.FTBTranscendentalRodiniumArmorGUI = Foundation.OverrideDef("FTB Transcendental Rodinium Armor GUI", "Tactical.Interface.ShipDisplay.SetShipID", __name__ ".SetShipID", FoundationTech.dMode)
# 
