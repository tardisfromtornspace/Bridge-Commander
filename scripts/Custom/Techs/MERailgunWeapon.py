...
#
# Start on 2:
# This tech is meant to adjust some ME Tau'ri Railgun Weapons to behave a bit more like in the Stargate show, alongside its own sub-technologies that makes the behaviour even more canonical.
# On this case, the tech is here to adjust to the fact these weapons are super-effective against unshielded vessels but deal little damage to shielded vessels, at least those with ME shields.
# Additionally, it is added as a way to make another difference between ME Shields/Hulls and ST Shields/Hulls regarding Railgun weapons:
...
# If you want an new specific subTech that modifies part of the ME Railgun Effect, you can do it by adding a file under the scripts\Custom\Techs\MERailgunWeaponScripts directory; if possible with a reasonable name related to the Technology(ies) it covers. 
...
# The reason we make a tech for this, is because ME Shields behave differently from ST, SW shields or B5 defences against a Railgun Weapon.
# On this case, the tech is here to adjust to the fact ME Railgun weapons are super-effective against unshielded vessels but deal little damage to shielded vessels, at least those with ME shields.
...
RailgunSGVulnerableShields = [] # No actual SG vessel is vulnerable to Tau'ri Railgun cannons nor ME weapons in the way the Tau'ri were to Tollan or Asgard Ion ones. Just leaving this here in case someone randomly wants to add that for a fanmade ship or something
...
global lImmuneMERailgunWeaponShips
lImmuneMERailgunWeaponShips = []
...
# Alongside these parameters, there are additional ones, but will be ignored unless they are on the "BasicMERailgunWeaponConfiguration.py" subTech file:
...
def LoadExtraLimitedPlugins(dExcludePlugins=_g_dExcludeSomePlugins):
	...
	dir="scripts\\Custom\\Techs\\MERailgunWeaponScripts" # I want to limit any vulnerability as much as I can while keeping functionality
	import string
	...
	for plugin in list:
		...
		if (extension == "py") and not fileName == "__init__": # I am not allowing people to just use the .pyc directly, I don't want people to not include source scripts - Alex SL Gato
			#print "MERailgunWeapon script is reviewing " + fileName + " of dir " + dir
				...
				if not filesChecked.has_key(fileName):
					...
					if hasattr(banana, "legacyImmunity"): # These will not be affected at all by the hull or shield effects, period
						...
						for item in banana.legacyImmunity:
							lImmuneMERailgunWeaponShips.append(item)
					...
					if fileName == "MERailgunWeaponBasicConfiguration":
					...
					#print "MERailgunWeapon reviewing of this subtech is a success"
			except:
				print "someone attempted to add more than they should to the MERailgunWeapon script"
				traceback.print_exc()
...
try:
	...
	class MERailgunWeaponTorpedo(FoundationTech.TechDef):
		...
		def IsMERailgunWeaponYield(self):
			return 1

		def IsSGRailgunWeaponYield(self): # Same mechanics, so make it trigger for both
			return 1
		...
		def OnYield(self, pShip, pInstance, pEvent, pTorp):
			...
			global lImmuneMERailgunWeaponShips # This legacy list goes first - it is to reduce unnecessary calculations
			if sShipScript in lImmuneMERailgunWeaponShips:
				return
			...
			if(pEvent.IsHullHit()):
				...
				for item in variableNames.keys():
					if variableNames[item].has_key("interactionHullBehaviour"): # These are reserved for when the hull has been hit! These are meant to be accumulative, for defenses.
						...
						try:
							...
							print "Some MERailgunWeapon hull subtech suffered an error"
							traceback.print_exc()
				...
				global SlowDownRatio
				mod = "Tactical.Projectiles.ExtraPhasedDamageDummyMERailgun" # This torpedo was made so Automated Point Defence scripts stop harrasing us! 
				try:
					...
					if pTempTorp and hasattr(pTempTorp, "SetLifetime"):
						pTempTorp.SetLifetime(5.0)			
				except:
					print "You are missing 'Tactical.Projectiles.ExtraPhasedDamageDummyMERailgun' torpedo on your install, without that the ME Railgun Weapons here cannot deal extra hull damage... or another error happened"
					traceback.print_exc()
	...
	oMERailgunWeaponTorp = MERailgunWeaponTorpedo("MERailgunWeapon Torpedo")
	# Just a few standard torps I know of that are MERailgunWeapon... 

except:
	print "FoundationTech, or the FTB mod, or both are not installed, \nMERailgunWeapon is there but NOT enabled or present in your current BC installation"
	traceback.print_exc()
