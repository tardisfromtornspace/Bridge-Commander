...
# This file takes care of how B5 defences are affected by SG Plasma Weapons.
# Babylon 5 - between the fact that lore-wise Babylon 5 has no shields except Vree, Abbai, Vorlons, potentially other First Ones, and Thirdspace Aliens; and that mod-wise the latest Babylon 5 super-pack mods already have alternatives of defense against these weapons such as Simulated Point Defense, and that for certain vessels a semblance of a shield was kept for blocking or imitating phaser absorption, we must guarantee that some Plasma Weapons can still surpass that token shield threshold in some way to work as intended for certain vessels.
...
global PlasmaB5LegacyShieldDamageMultiplier
PlasmaB5LegacyShieldDamageMultiplier = 5.0 
...
defenceGridMultiplier = 0.9 # Used for legacy reasons
hullPolarizerMultiplier = 0.65
shadowDispersiveHullMultiplier = 0.88
shadowDispersiveHullMin = 0.49
...
def interactionShieldBehaviour(pShip, sScript, sShipScript, pInstance, pEvent, pTorp, pInstancedict, pAttackerShipID, hullDamageMultiplier, shieldDamageMultiplier, shouldPassThrough, considerPiercing, shouldDealAllFacetDamage, wasChanged):
	...
	if pInstance:
		if pInstancedict.has_key('Defense Grid') or pInstancedict.has_key('Gravimetric Defense'): # ohhhh Babylon 5 defence grids and gravimetric defenses - yeah, no shields, and if you already reached this point is because your point defence system failed to take this shot, sorry
			...
			shieldDamageMultiplier = shieldDamageMultiplier * PlasmaB5LegacyShieldDamageMultiplier
		...
		elif pInstancedict.has_key("Vree Shields"): # Vree/Abbai/Shinindrea shields work very differently, they may care little about your weapon being Plasma...
			wasChanged = wasChanged + 1
