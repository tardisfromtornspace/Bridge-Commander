...
class PhotonicCannon(FoundationTech.TechDef):
	def __init__(self, name):
		...
		#print "Initialized PhotonicCannon"

	def Attach(self, pInstance):
		pShip = App.ShipClass_Cast(App.TGObject_GetTGObjectPtr(pInstance.pShipID))
		if pShip != None:
			pInstance.__dict__['PhotonicCannon'] = {}
			dMasterDict = pInstance.__dict__['PhotonicCannon']
		else:
			pass
			#print "PhotonicCannon Error (at Attach): couldn't acquire ship of id", pInstance.pShipID
	...
	def Detach(self, pInstance):
		...
		if pShip != None:
			...
			#print "PhotonicCannon Error (at Detach): couldn't acquire ship of id", pInstance.pShipID
			pass
	...
	def PhaserStartedHitting(self, pEvent):
		...
		try:
			...
			return
		if not pInstance.__dict__.has_key("PhotonicCannon"):
		...
		if not pInstance.__dict__['PhotonicCannon'].has_key(sTargetName):
			pInstance.__dict__['PhotonicCannon'][sTargetName] = {"HittingNames": [], "Target": pTarget}
			...
			pTimer.SetTimerStart( App.g_kUtopiaModule.GetGameTime()+4.5 )
			pTimer.SetDelay(0)
			pTimer.SetDuration(0)
			pTimer.SetEvent(pEvent)
			App.g_kTimerManager.AddTimer(pTimer)

			pInstance.__dict__['PhotonicCannon'][sTargetName]['Timer'] = pTimer
		...
		if not pPhaser.GetName() in pInstance.__dict__['PhotonicCannon'][sTargetName]['HittingNames']:
			pInstance.__dict__['PhotonicCannon'][sTargetName]['HittingNames'].append( pPhaser.GetName() )
	...
	def PhaserStoppedHitting(self, pEvent):
		...
		if not pInstance.__dict__['PhotonicCannon'].has_key(sTargetName):
			...
			return
		if pPhaser.GetName() in pInstance.__dict__['PhotonicCannon'][sTargetName]['HittingNames']:
			pInstance.__dict__['PhotonicCannon'][sTargetName]['HittingNames'].remove( pPhaser.GetName() )
			#print "FSGP (Stop): removing phaser", pPhaser.GetName(), "from target hitting list."

		if len(pInstance.__dict__['PhotonicCannon'][sTargetName]['HittingNames']) <= 0:
			#5-secs hitting sequence aborted... cancel it.
			App.g_kTimerManager.DeleteTimer(pInstance.__dict__['PhotonicCannon'][sTargetName]['Timer'].GetObjID())
			pInstance.__dict__['PhotonicCannon'][sTargetName]['Timer'] = None
			del pInstance.__dict__['PhotonicCannon'][sTargetName]
	...
	def HandleInstaKill(self, pEvent):
		...
		if not pInstance.__dict__.has_key("PhotonicCannon"):
			...
			return
		if pInstance.__dict__['PhotonicCannon'].has_key(sTargetName):
			#okydokey, we did it. Blow up the ship. then delete our data about it.
			pTarget = pInstance.__dict__['PhotonicCannon'][sTargetName]['Target']
			pTarget.DestroySystem(pTarget.GetSensorSubsystem())
			pTarget.DestroySystem(pTarget.GetRepairSubsystem())
			pTarget.DestroySystem(pTarget.GetMainBatterySubsystem())
			App.g_kTimerManager.DeleteTimer(pInstance.__dict__['PhotonicCannon'][sTargetName]['Timer'].GetObjID())
			pInstance.__dict__['PhotonicCannon'][sTargetName]['Timer'] = None
			del pInstance.__dict__['PhotonicCannon'][sTargetName]
...
oPhotonicCannon = PhotonicCannon("PhotonicCannon")