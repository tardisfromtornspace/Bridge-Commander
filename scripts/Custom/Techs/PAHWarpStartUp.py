...
class PAHWarpStartUpDef(FoundationTech.TechDef):
	def AttachShip(self, pShip, pInstance):
		...
		if pShip and getattr(pInstance, self.name, None):
			...
			if not getattr(pInstance, "bAddedPAHWarpHandlers", 0):
				...
				pInstance.bAddedPAHWarpHandlers = 1

	def DetachShip(self, pShipID, pInstance):
		pShip = App.ShipClass_Cast(App.TGObject_GetTGObjectPtr(pInstance.pShipID))
		if pShip and getattr(pInstance, "bAddedPAHWarpHandlers", 0):
			...
			pInstance.bAddedPAHWarpHandlers = 0


oPAHWarpStartUp = PAHWarpStartUpDef("PAHWarpStartUp")
...
def StartingWarp(pObject, pEvent):
	...
	dConfig = getattr(pInstance, "PAHWarpStartUp")
	if not dConfig:
		return
