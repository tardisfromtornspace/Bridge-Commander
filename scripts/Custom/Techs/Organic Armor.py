...
class AblativeDef(FoundationTech.GaugeTechDef):
	...
	def OnDefense(self, pShip, pInstance, oYield, pEvent):
		repair = pInstance.__dict__['Organic Armor']
		...
		if repair > 0.0:
			...
			if str(pInstance.__dict__['Organic Armor'])[0] == "[":
				pInstance.__dict__['Organic Armor'][0] = repair = repair - pEvent.GetDamage()
				
			else:
				pInstance.__dict__['Organic Armor'] = repair = repair - pEvent.GetDamage()
...
oAblative = AblativeDef('Organic Armor')
...
# 	#		if pInstance.__dict__.has_key("Organic Armor"):
# 	 	pIterator = pShip.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
# 		pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
# 		while pSubsystem != None:
# 			if pSubsystem.GetName() == "Organic Armor":
# 				pArmour = pSubsystem
# 				pShip.EndGetSubsystemMatch(pIterator)
# 				break
# 			pSubsystem = pShip.GetNextSubsystemMatch(pIterator)
...
# #Foundation.OverrideDef.FTBOrganicArmorGUI = Foundation.OverrideDef("FTB Organic Armor GUI", "Tactical.Interface.ShipDisplay.SetShipID", __name__ ".SetShipID", FoundationTech.dMode)
# 
