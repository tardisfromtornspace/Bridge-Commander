...
# Special fields:
# - "Distance": minimum distance for the effect to start reducing... until reching 0. For torps, it's the minimum distance between being accurate and missing. Default is 30 km.
# - "Sensor": at this value or below, the attacker will have its attacks scrambled. Default is 100.
# - "Miss": this value will indicate by how much they will miss. Default is 0.5
"""
...
globalPrimitiveSensor = 100.0
globalfMissExtra = 0.5
globalStartAcute = 30.0
...
def commonFunction(techName, pInstanceFool, fMiss, pShip, pAttackerInstance, pTarget, pDefenderInstance, pTorp, fSensorRange, fAngleDiff, fObjectDistance):
	...
	redDis = globalStartAcute
	if pInstanceFool:
		if pInstanceFool[techName].has_key("Sensor"):
			primitiveSensor = pInstanceFool[techName]["Sensor"]
		if pInstanceFool[techName].has_key("Distance"):
			redDis = pInstanceFool[techName]["Distance"]
	fMissExtra = 0.0
	try:
		extraMultiplier = 1.0
		...
		if extraMultiplier * effortMultiplier * primitiveSensor > fSensorRange:
			...
			if fObjectDistance < redDis/ticksPerKilometer:
				fMissExtra = fMissExtra * fObjectDistance/redDis * ticksPerKilometer
...
def pulseTCondition(techName, pInstanceFool, fMiss, pAttackerShip, pAttackerInstance, pDefenderShip, pDefenderInstance, pTorp): # Establishes the minimum number of strikes before it adapts, stacking along the random value above - please notice some weapons deal multi-strikes. Values must be >= 0
	...
	if pDefenderShip:
		...
		redDis = globalStartAcute
		if pInstanceFool and pInstanceFool[techName].has_key("Distance"):
			redDis = pInstanceFool[techName]["Distance"]

		if fObjectDistance < redDis/ticksPerKilometer:
			fObjectDistance = 0.0

		return commonFunction(techName, pInstanceFool, fMiss, pAttackerShip, pAttackerInstance, pDefenderShip, pDefenderInstance, pTorp, fSensorRange, None, fObjectDistance)
	else:
		return 0.0