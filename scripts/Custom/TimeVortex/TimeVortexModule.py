...
from Custom.TimeVortex.Libs import LoadFlash
from Custom.TimeVortex.Libs import AimForGoodPos
from Custom.TimeVortex.Libs import AimForGoodPosExit
from Custom.TimeVortex.Libs import EngineStats
from Custom.TimeVortex.Libs import EngineStatsETA
from Custom.TimeVortex.Libs import EngineStatsIdealETA
from Custom.TimeVortex.Libs import EngineStatsMenuCalc
...
# List of hull properties TimeVortex looks for
sTimeVortexList = ['TimeVortex Drive 1', 'TimeVortex Drive 2', 'TimeVortex Drive 3', 'TimeVortex Drive 4', 'TimeVortex Drive 5', 'TimeVortex Drive 6', 'TimeVortex Drive 7', 'TimeVortex Drive 8', 'TimeVortex Drive 9', 'TimeVortex Drive 10', 'TimeVortex Drive 11', 'TimeVortex Drive 12', 'TimeVortex Drive 13', 'TimeVortex Drive 14', 'TimeVortex Drive 15', 'TimeVortex Drive 16', 'TimeVortex Drive 17', 'TimeVortex Drive 18', 'TimeVortex Drive 19', 'TimeVortex Drive 20']
...
# Path
sPath = "scripts\\Custom\\TimeVortex\\Libs\\EngineStats.py"
sPathETA = "scripts\\Custom\\TimeVortex\\Libs\\EngineStatsETA.py"
sPathIdealETA = "scripts\\Custom\\TimeVortex\\Libs\\EngineStatsIdealETA.py"
sPathStats = "scripts\\Custom\\TimeVortex\\Libs\\EngineStatsMenuCalc.py"

# Tunnel Engine noise is hardcoded and not customizable
pTimeVortexEngineSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/TimeVortexenginenoise.wav", "TimeVortexEngineSound", 0)
pTimeVortexEngineSound.SetSFX(0) 
pTimeVortexEngineSound.SetInterface(1)
pTimeVortexEngineSound.SetLooping(1)


# Main function called by the zzzTimeVortexMode.py
def init():
        ...
        # Navpoint exists?
        try:
            pSet.DeleteObjectFromSet("TimeVortex Navpoint")
        except:
            pass
        ...
        TimeVortexInstalled = HasTimeVortex(pPlayer)
        if not TimeVortexInstalled == 'TimeVortex Installed':
            return

        # Create menus and buttons
        bMenu = DS9FXMenuLib.CreateMenu("TimeVortex Details", "Helm", 2, 0)
        bButton = DS9FXMenuLib.CreateButton("TimeVortex Drive", "Helm", __name__ + ".TimeVortexGUI", "TimeVortex Details")
        bInterceptMenu = App.STMenu_CreateW(App.TGString("Intercept Options"))
	bMenu.PrependChild(bInterceptMenu)
        bInterceptButton = DS9FXMenuLib.CreateBridgeMenuButton("TimeVortex Intercept", "Helm", __name__ + ".TimeVortexInterceptStats", bInterceptMenu)
        bNavPointButton = DS9FXMenuLib.CreateBridgeMenuButton("Initiate Navpoint Mode", "Helm", __name__ + ".TimeVortexNavpoint", bInterceptMenu)
	...
	bNavPointButton.SetNormalColor(NormalColor2)
        ...
        # Resolve the small issue when the warp button is enabled and TimeVortex is not
...
def handlers():
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        if pMission.GetScript() == "QuickBattle.QuickBattle":
            print 'TimeVortex: Quick Battle is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
        elif pMission.GetScript() == "Custom.QuickBattleGame.QuickBattle":
            print 'TimeVortex: Quick Battle Replacement is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
	else:
	    print 'TimeVortex: Unsupported game type is running... Mod is shutting down...'
	    pSupported = 0
	    return
...
def quitting():
        ...
        try:
            App.g_kSoundManager.StopSound("TimeVortexEngineSound")
        except:
            pass
        try:
        ...
        try:
            ...
            pass
        try:
            App.g_kEventManager.RemoveBroadcastHandler(ET_ENGAGE, pMission, __name__ + ".TimeVortexStats")
        except:
            pass
        try:
...
def CourseSet(pObject, pEvent):
        ...
        if not (pEvent.GetInt() == App.CharacterClass.EST_SET_COURSE_INTERCEPT):
            ...
            # Incompatibility fix for hyperdrive & TimeVortex
...
# Create a navpoint
def TimeVortexNavpoint(pObject, pEvent):
...
def MoveNavpoint(pObject, pEvent):
        ...
        fNav = MissionLib.GetShip("TimeVortex Navpoint", pSet) 
        fNav.SetTranslateXYZ(pLoc.GetX() + cordX, pLoc.GetY() + cordY, pLoc.GetZ() + cordZ)
        fNav.UpdateNodeOnly()


# The newest addition to the TimeVortex family in system TimeVortex intercept, fast as hell on great distances
def TimeVortexInterceptStats(pObject, pEvent):
        global lStats
	...
	# Add the stats of each engine into the list
        ...
        nt.write(file, "# Do not Edit this file, TimeVortex uses it to store data\n")
        i = 0
        # Write stats into the file
        for stat in lStats:
            i = i + 1
            nt.write(file, "\nTimeVortexDrive" + str(i) + " = " + str(stat))
        ...
        iTotal = EngineStats.TimeVortexDrive1 + EngineStats.TimeVortexDrive2 + EngineStats.TimeVortexDrive3 + EngineStats.TimeVortexDrive4 + EngineStats.TimeVortexDrive5 + EngineStats.TimeVortexDrive6 + EngineStats.TimeVortexDrive7 + EngineStats.TimeVortexDrive8 + EngineStats.TimeVortexDrive9  + EngineStats.TimeVortexDrive10 + EngineStats.TimeVortexDrive11 + EngineStats.TimeVortexDrive12 + EngineStats.TimeVortexDrive13 + EngineStats.TimeVortexDrive14 + EngineStats.TimeVortexDrive15 + EngineStats.TimeVortexDrive16 + EngineStats.TimeVortexDrive17 + EngineStats.TimeVortexDrive18 + EngineStats.TimeVortexDrive19 + EngineStats.TimeVortexDrive20 

        pTotal = iTotal / nTotal

        # Can't jump if the engines are below 1%
        if pTotal < 1.0:
            print 'TimeVortex: TimeVortex Engines are badly damaged, cannot intercept target'
            return
...
def InterceptTargetCheck():
        ...
        if pTargetPlanet:
            if not DistanceCheck(pTargetPlanet) >= 400:
                ...
                pSequence.Play()
                # print 'TimeVortex: Planet is too close...'
                return
            ...
            if not DistanceCheck(pTargetShip) >= 250:
                ...
                pSequence.AddAction(pAction, None, 1)
                pSequence.Play()
                # print 'TimeVortex: Ship is too close...'
                return
...
def ResetButtonString(pAction, pButton):
        try:
            pButton.SetName(App.TGString("TimeVortex Intercept"))
        except:
            pass
...
def InterceptTarget(pTarget, IsPlanet):
        ...
        pWasCloaked = 0
        if pPlayer.IsCloaked() and not (HasPhasedTimeVortex(pPlayer) == "Phased TimeVortex Installed"):
            pWasCloaked = 1
...
def AdditionalPosAndCheckIntercept(pAction, pTarget, IsPlanet):
        ...
        if sAim == 'TRUE':               
                ...
                pEntro = "TimeVortexFlash"
                pSalgo = "TimeVortexFlashExit"
                pSonidoEntro = "EnteringFlash"
                pSonidoSalgo = "ExitingFlash"
                if (HasSigmaTimeVortex(pPlayer) == "Sigma Walkers TimeVortex Installed"):
                    pEntro = "SigmaFlash"
                    pSalgo = "SigmaFlash"
                if (HasPhasedTimeVortex(pPlayer) == "Phased TimeVortex Installed"):
...
def InaccionEntry():
        pEnterSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/dummy.wav", "Enter", 0)
...
def InaccionExit():
        pEnterSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/dummy.wav", "Exit", 0)
        pEnterSound.SetSFX(0) 
        pEnterSound.SetInterface(1)
...
def TimeVortexGUI(pObject, pEvent):
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        # Setup Events
	App.g_kEventManager.AddBroadcastPythonFuncHandler(ET_ENGAGE, pMission, __name__ + ".TimeVortexStats")
...
def CreateEntries(pObject, pEvent):
        ...
        pOptionsWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Engage TimeVortex"), 0.0, 0.0, None, 1, 0.19, 0.19, App.g_kMainMenuBorderMainColor)
...
def ReturnETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsETA.TimeVortexDrive1 + EngineStatsETA.TimeVortexDrive2 + EngineStatsETA.TimeVortexDrive3 + EngineStatsETA.TimeVortexDrive4 + EngineStatsETA.TimeVortexDrive5 + EngineStatsETA.TimeVortexDrive6 + EngineStatsETA.TimeVortexDrive7 + EngineStatsETA.TimeVortexDrive8 + EngineStatsETA.TimeVortexDrive9  + EngineStatsETA.TimeVortexDrive10 + EngineStatsETA.TimeVortexDrive11 + EngineStatsETA.TimeVortexDrive12 + EngineStatsETA.TimeVortexDrive13 + EngineStatsETA.TimeVortexDrive14 + EngineStatsETA.TimeVortexDrive15 + EngineStatsETA.TimeVortexDrive16 + EngineStatsETA.TimeVortexDrive17 + EngineStatsETA.TimeVortexDrive18 + EngineStatsETA.TimeVortexDrive19 + EngineStatsETA.TimeVortexDrive20 
...
def ReturnIdealETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsIdealETA.TimeVortexDrive1 + EngineStatsIdealETA.TimeVortexDrive2 + EngineStatsIdealETA.TimeVortexDrive3 + EngineStatsIdealETA.TimeVortexDrive4 + EngineStatsIdealETA.TimeVortexDrive5 + EngineStatsIdealETA.TimeVortexDrive6 + EngineStatsIdealETA.TimeVortexDrive7 + EngineStatsIdealETA.TimeVortexDrive8 + EngineStatsIdealETA.TimeVortexDrive9  + EngineStatsIdealETA.TimeVortexDrive10 + EngineStatsIdealETA.TimeVortexDrive11 + EngineStatsIdealETA.TimeVortexDrive12 + EngineStatsIdealETA.TimeVortexDrive13 + EngineStatsIdealETA.TimeVortexDrive14 + EngineStatsIdealETA.TimeVortexDrive15 + EngineStatsIdealETA.TimeVortexDrive16 + EngineStatsIdealETA.TimeVortexDrive17 + EngineStatsIdealETA.TimeVortexDrive18 + EngineStatsIdealETA.TimeVortexDrive19 + EngineStatsIdealETA.TimeVortexDrive20 

        pTotal = iTotal / nTotal

        # Damn, all of this to get that... It's finally done!!!
        pTimerIdealETA = 9000.0 / pTotal
...
def ReturnEngineStats():
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsMenuCalc.TimeVortexDrive1 + EngineStatsMenuCalc.TimeVortexDrive2 + EngineStatsMenuCalc.TimeVortexDrive3 + EngineStatsMenuCalc.TimeVortexDrive4 + EngineStatsMenuCalc.TimeVortexDrive5 + EngineStatsMenuCalc.TimeVortexDrive6 + EngineStatsMenuCalc.TimeVortexDrive7 + EngineStatsMenuCalc.TimeVortexDrive8 + EngineStatsMenuCalc.TimeVortexDrive9  + EngineStatsMenuCalc.TimeVortexDrive10 + EngineStatsMenuCalc.TimeVortexDrive11 + EngineStatsMenuCalc.TimeVortexDrive12 + EngineStatsMenuCalc.TimeVortexDrive13 + EngineStatsMenuCalc.TimeVortexDrive14 + EngineStatsMenuCalc.TimeVortexDrive15 + EngineStatsMenuCalc.TimeVortexDrive16 + EngineStatsMenuCalc.TimeVortexDrive17 + EngineStatsMenuCalc.TimeVortexDrive18 + EngineStatsMenuCalc.TimeVortexDrive19 + EngineStatsMenuCalc.TimeVortexDrive20 
...
def TimeVortexStats(pObject, pEvent):
	...
	# Add the stats of each engine into the list
        ...
        # Can't jump if the engines are below 1%
        if pTotal < 1.0:
            print 'TimeVortex: TimeVortex Engines are badly damaged, cannot initiate the jump'
            return

        # Damn, all of this to get that... It's finally done!!!
        pTimer = 9000.0 / pTotal
...
# Check if the ship is actually equiped with TimeVortex drive
def HasTimeVortex(pShip):        
        # Grab values
...
# Checks if this ship can do phased TimeVortex
def HasPhasedTimeVortex(pPlayer):
        pIterator = pPlayer.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
	pSystem = pPlayer.GetNextSubsystemMatch(pIterator)

	# Add the stats of each engine into the list
        while (pSystem != None):
		if pSystem.GetName() == "Good TimeVortex Drive":
                   pStats = pSystem.GetConditionPercentage() * 100.0
                   pDisabled = pSystem.GetDisabledPercentage() * 100.0
                   if pStats > pDisabled:
...
def HasSigmaTimeVortex(pPlayer):
        pIterator = pPlayer.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
	pSystem = pPlayer.GetNextSubsystemMatch(pIterator)
	...
	# Add the stats of each engine into the list
		...
		if pSystem.GetName() == "TransDimensional Drive":
                   ...
                   if pStats > pDisabled:
                       # Engine is enabled, means it works
                       return 'Sigma Walkers TimeVortex Installed'
                       break                
		pSystem = pPlayer.GetNextSubsystemMatch(pIterator)

        pPlayer.EndGetSubsystemMatch(pIterator)

# TimeVortex sequence initiated
...
def EnteringFlash(pAction):
        ...
        if hasattr(pModule, "TimeVortexCustomizations"):
            ...
            if pCustomization.has_key('EntrySound'):
                pSound = "scripts/Custom/TimeVortex/SFX/" + pCustomization['EntrySound']
            ...
            # No sound entry found
                ...
                pEnterSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/enterTimeVortex.wav", "Enter", 0)
                pEnterSound.SetSFX(0) 
                pEnterSound.SetInterface(1)
...
def ShadowFlashExit(pAction):
        pEnterSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/exittimeVortex.wav", "Exit", 0)
        pEnterSound.SetSFX(0) 
        pEnterSound.SetInterface(1)
...
def ExitingFlash(pAction):
        ...
        if hasattr(pModule, "TimeVortexCustomizations"):
            ...
            if pCustomization.has_key('ExitSound'):
                pSound = "scripts/Custom/TimeVortex/SFX/" + pCustomization['ExitSound']
            ...
            # No sound entry found
            	...
            	pExitSound = App.TGSound_Create("scripts/Custom/TimeVortex/SFX/exitTimeVortex.wav", "Exit", 0)
            	pExitSound.SetSFX(0) 
            	pExitSound.SetInterface(1)
...
def MoveForwardAI(pAction):
        pPlayer = MissionLib.GetPlayer()
        import Custom.TimeVortex.Libs.DS9FXWormholeExitingSeqAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.TimeVortex.Libs.DS9FXWormholeExitingSeqAI.CreateAI(pPlayerCast))
...
def InitiateSearchAI():
        pPlayer = MissionLib.GetPlayer()
        import Custom.TimeVortex.Libs.TimeVortexSearchSafePosition
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.TimeVortex.Libs.TimeVortexSearchSafePosition.CreateAI(pPlayerCast))
...
def ClearAIs(pAction):        
        pPlayer = MissionLib.GetPlayer()        
        import Custom.TimeVortex.Libs.DS9FXStayAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.TimeVortex.Libs.DS9FXStayAI.CreateAI(pPlayerCast))
...
def BackToNormal(pAction):
	...
	# Restore things back to normal
        ...
        if pWasCloaked == 1 and not (HasPhasedTimeVortex(pPlayer) == "Phased TimeVortex Installed"):
            pCloak = pPlayer.GetCloakingSubsystem()
            if pCloak:
                pCloak.InstantCloak()
...
def TimeVortexFlash(pAction):
        ...
        return 0
def TimeVortexFlashExit(pAction):
        # print 'TimeVortex exit should be starting now...'    
        pPlayer = MissionLib.GetPlayer()
...
def Engage(pObject, pEvent):
        ...
        # Resolve the dark screen problem after a new object is created in the TimeVortex\hyperdrive set
        pGame = App.Game_GetCurrentGame()
	pEpisode = pGame.GetCurrentEpisode()
	pMission = pEpisode.GetCurrentMission()
	...
	TacticalInterfaceHandlers.ClearTarget(None, None)
        ...
        if App.g_kSetManager.GetSet("TimeVortexTunnel1"):
                ...
                pDummySet = App.g_kSetManager.GetSet("TimeVortexTunnel1")
        ...
        for kShip in pSet.GetClassObjectList(App.CT_SHIP):
                ...
                # TimeVortex ability?!
                TimeVortexInstalled = HasTimeVortex(fShip)
                if not TimeVortexInstalled == 'TimeVortex Installed':
        ...
        pTunnel = loadspacehelper.CreateShip("timeVortex", pDummySet, TunnelString, "Player Start")
        ...
        pTunnel2 = loadspacehelper.CreateShip("timeVortex", pDummySet, TunnelString2, "Player Start")
	...
	# Custom tunnel textures?
        ...
        if TimeVortexConfiguration.LoopSound == 1:
            # Play the loop sound
            App.g_kSoundManager.PlaySound("TimeVortexEngineSound")

        if TimeVortexConfiguration.Prints == 1:
        ...
        if hasattr(pModule, "TimeVortexCustomizations"):
            ...
            if pCustomization.has_key('BackdropTexture'):
                ...
                if pCustomization['BackdropTexture'] == "Off":
                    ...
                    Backdrop = "scripts/Custom/TimeVortex/GFX/" + pCustomization['BackdropTexture']
            ...
            # There's no gfx defined, use the user specified defaults
                ...
                if TimeVortexConfiguration.BackdropGFX == 'Default':
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
                    ...
                    kUp.SetXYZ(0.049823, 0.254099, 0.965894)
                    kThis.AlignToVectors(kForward, kUp)
                    kThis.SetTextureFileName("data/timeVortexback.tga")
                    kThis.SetTargetPolyCount(256)
                    kThis.SetHorizontalSpan(1.000000)
                    kThis.SetVerticalSpan(1.000000)
                    kThis.SetSphereRadius(300.000000)
                    ...
                    kThis.Update(0)
                    kThis = None
                elif TimeVortexConfiguration.BackdropGFX == 'Off':
                    pass
                else:
                    Backdrop = "scripts/Custom/TimeVortex/GFX/" + TimeVortexConfiguration.BackdropGFX
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
...
def AIExit(pObject, pEvent):
        ...
        # Now the magic happens
        pPlayer = MissionLib.GetPlayer()
        pSet = App.g_kSetManager.GetSet('TimeVortexTunnel1')
        ...
        pSet.DeleteObjectFromSet("TimeVortex Outer")
        pSet.DeleteObjectFromSet("TimeVortex Inner")
		...
		pLocation = pPlayer.GetWorldLocation() 
                ...
                # Is there a customization for this ship available?
                if hasattr(pModules, "TimeVortexCustomizations"):
                    pCustomization = pModules.TimeVortexCustomizations()
...
# When a new object is created in the TimeVortex set it collides with the massive tunnel thus darkening the screen
def FixDarkScreen(pObject, pEvent):
        pPlayer = App.Game_GetCurrentPlayer()    
        pSet = pPlayer.GetContainingSet()

        if (pSet.GetName() == "TimeVortexTunnel1"):            
            fTunnel = MissionLib.GetShip("TimeVortex Outer", pSet) 
            fTunnel2 = MissionLib.GetShip("TimeVortex Inner", pSet) 
...
def FixDarkScreenExplosionsDelay(pObject, pEvent):
        ...
        # Create the tunnel
        TunnelString = "TimeVortex Outer"
        pTunnel = loadspacehelper.CreateShip("timeVortex", pSet, TunnelString, "Player Start")
        ...
        # Create the tunnel
        TunnelString2 = "TimeVortex Inner"
        pTunnel2 = loadspacehelper.CreateShip("timeVortex", pSet, TunnelString2, "Player Start")
...
def RemoveMenu():
        ...
        pMenu = pHelm.GetMenu()
	if (pMenu != None):
		pButton = pMenu.GetSubmenu("TimeVortex Details")
		if (pButton != None):
