...
import nt
import string
from Custom.Slipstream.Libs import DS9FXMenuLib
from Custom.Slipstream.Libs import LoadFlash
from Custom.Slipstream.Libs import AimForGoodPos
from Custom.Slipstream.Libs import AimForGoodPosExit
from Custom.Slipstream.Libs import EngineStats
from Custom.Slipstream.Libs import EngineStatsETA
from Custom.Slipstream.Libs import EngineStatsIdealETA
from Custom.Slipstream.Libs import EngineStatsMenuCalc
...
# List of hull properties Slipstream looks for
sSlipstreamList = ['Slipstream Drive 1', 'Slipstream Drive 2', 'Slipstream Drive 3', 'Slipstream Drive 4', 'Slipstream Drive 5', 'Slipstream Drive 6', 'Slipstream Drive 7', 'Slipstream Drive 8', 'Slipstream Drive 9', 'Slipstream Drive 10', 'Slipstream Drive 11', 'Slipstream Drive 12', 'Slipstream Drive 13', 'Slipstream Drive 14', 'Slipstream Drive 15', 'Slipstream Drive 16', 'Slipstream Drive 17', 'Slipstream Drive 18', 'Slipstream Drive 19', 'Slipstream Drive 20']
...
# Path
sPath = "scripts\\Custom\\Slipstream\\Libs\\EngineStats.py"
sPathETA = "scripts\\Custom\\Slipstream\\Libs\\EngineStatsETA.py"
sPathIdealETA = "scripts\\Custom\\Slipstream\\Libs\\EngineStatsIdealETA.py"
sPathStats = "scripts\\Custom\\Slipstream\\Libs\\EngineStatsMenuCalc.py"

# Tunnel Engine noise is hardcoded and not customizable
pSlipstreamEngineSound = App.TGSound_Create("scripts/Custom/Slipstream/SFX/slipstreamenginenoise.wav", "SlipstreamEngineSound", 0)
pSlipstreamEngineSound.SetSFX(0) 
pSlipstreamEngineSound.SetInterface(1)
pSlipstreamEngineSound.SetLooping(1)


# Main function called by the zzzSlipstreamMode.py
def init():
        ...
        # Navpoint exists?
        try:
            pSet.DeleteObjectFromSet("Slipstream Navpoint")
        except:
            pass
        ...
        SlipstreamInstalled = HasSlipstream(pPlayer)
        if not SlipstreamInstalled == 'Slipstream Installed':
            return

        # Create menus and buttons
        bMenu = DS9FXMenuLib.CreateMenu("Slipstream Details", "Helm", 2, 0)
        bButton = DS9FXMenuLib.CreateButton("Slipstream Drive", "Helm", __name__ + ".SlipstreamGUI", "Slipstream Details")
        bInterceptMenu = App.STMenu_CreateW(App.TGString("Intercept Options"))
	bMenu.PrependChild(bInterceptMenu)
        bInterceptButton = DS9FXMenuLib.CreateBridgeMenuButton("Slipstream Intercept", "Helm", __name__ + ".SlipstreamInterceptStats", bInterceptMenu)
        bNavPointButton = DS9FXMenuLib.CreateBridgeMenuButton("Initiate Navpoint Mode", "Helm", __name__ + ".SlipstreamNavpoint", bInterceptMenu)
	...
	bNavPointButton.SetNormalColor(NormalColor2)
        ...
        # Resolve the small issue when the warp button is enabled and slipstream is not
...
def handlers():
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        if pMission.GetScript() == "QuickBattle.QuickBattle":
            print 'Slipstream: Quick Battle is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
        elif pMission.GetScript() == "Custom.QuickBattleGame.QuickBattle":
            print 'Slipstream: Quick Battle Replacement is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
	else:
	    print 'Slipstream: Unsupported game type is running... Mod is shutting down...'
	    pSupported = 0
	    return
...
def quitting():
        ...
        try:
            ...
            pass
        try:
            App.g_kEventManager.RemoveBroadcastHandler(ET_ENGAGE, pMission, __name__ + ".SlipstreamStats")
        except:
            pass
        try:
...
def MoveNavpoint(pObject, pEvent):
        ...
        fNav = MissionLib.GetShip("Slipstream Navpoint", pSet) 
        fNav.SetTranslateXYZ(pLoc.GetX() + cordX, pLoc.GetY() + cordY, pLoc.GetZ() + cordZ)
        fNav.UpdateNodeOnly()


# The newest addition to the slipstream family in system slipstream intercept, fast as hell on great distances
def SlipstreamInterceptStats(pObject, pEvent):
	...
	# Add the stats of each engine into the list
        while (pSystem != None):
		if pSystem.GetName() in sSlipstreamList:
        ...
        nt.write(file, "# Do not Edit this file, Slipstream uses it to store data\n")
        i = 0
        # Write stats into the file
        for stat in lStats:
            i = i + 1
            nt.write(file, "\nSlipstreamDrive" + str(i) + " = " + str(stat))
        ...
        iTotal = EngineStats.SlipstreamDrive1 + EngineStats.SlipstreamDrive2 + EngineStats.SlipstreamDrive3 + EngineStats.SlipstreamDrive4 + EngineStats.SlipstreamDrive5 + EngineStats.SlipstreamDrive6 + EngineStats.SlipstreamDrive7 + EngineStats.SlipstreamDrive8 + EngineStats.SlipstreamDrive9  + EngineStats.SlipstreamDrive10 + EngineStats.SlipstreamDrive11 + EngineStats.SlipstreamDrive12 + EngineStats.SlipstreamDrive13 + EngineStats.SlipstreamDrive14 + EngineStats.SlipstreamDrive15 + EngineStats.SlipstreamDrive16 + EngineStats.SlipstreamDrive17 + EngineStats.SlipstreamDrive18 + EngineStats.SlipstreamDrive19 + EngineStats.SlipstreamDrive20 

        pTotal = iTotal / nTotal

        # Can't jump if the engines are below 1%
        if pTotal < 1.0:
            print 'Slipstream: Slipstream Engines are badly damaged, cannot intercept target'
            return
...
def InterceptTargetCheck():
        ...
        if pTargetPlanet:
            if not DistanceCheck(pTargetPlanet) >= 400:
                ...
                pSequence.Play()
                # print 'Slipstream: Planet is too close...'
                return
            ...
            if not DistanceCheck(pTargetShip) >= 250:
                ...
                pSequence.AddAction(pAction, None, 1)
                pSequence.Play()
                # print 'Slipstream: Ship is too close...'
                return
...
def ResetButtonString(pAction, pButton):
        try:
            pButton.SetName(App.TGString("Slipstream Intercept"))
        except:
            pass
...
def AdditionalPosAndCheckIntercept(pAction, pTarget, IsPlanet):
        ...
        if sAim == 'TRUE':               
                ...
                pSequence.AddAction(pAction, None, 1.5)
                pAction = App.TGScriptAction_Create(__name__, "SlipstreamFlash")
...
def SlipstreamGUI(pObject, pEvent):
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        # Setup Events
	App.g_kEventManager.AddBroadcastPythonFuncHandler(ET_ENGAGE, pMission, __name__ + ".SlipstreamStats")
...
def CreateEntries(pObject, pEvent):
        ...
        pOptionsWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Engage Slipstream"), 0.0, 0.0, None, 1, 0.19, 0.19, App.g_kMainMenuBorderMainColor)
...
def ReturnETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsETA.SlipstreamDrive1 + EngineStatsETA.SlipstreamDrive2 + EngineStatsETA.SlipstreamDrive3 + EngineStatsETA.SlipstreamDrive4 + EngineStatsETA.SlipstreamDrive5 + EngineStatsETA.SlipstreamDrive6 + EngineStatsETA.SlipstreamDrive7 + EngineStatsETA.SlipstreamDrive8 + EngineStatsETA.SlipstreamDrive9  + EngineStatsETA.SlipstreamDrive10 + EngineStatsETA.SlipstreamDrive11 + EngineStatsETA.SlipstreamDrive12 + EngineStatsETA.SlipstreamDrive13 + EngineStatsETA.SlipstreamDrive14 + EngineStatsETA.SlipstreamDrive15 + EngineStatsETA.SlipstreamDrive16 + EngineStatsETA.SlipstreamDrive17 + EngineStatsETA.SlipstreamDrive18 + EngineStatsETA.SlipstreamDrive19 + EngineStatsETA.SlipstreamDrive20 
...
def ReturnIdealETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsIdealETA.SlipstreamDrive1 + EngineStatsIdealETA.SlipstreamDrive2 + EngineStatsIdealETA.SlipstreamDrive3 + EngineStatsIdealETA.SlipstreamDrive4 + EngineStatsIdealETA.SlipstreamDrive5 + EngineStatsIdealETA.SlipstreamDrive6 + EngineStatsIdealETA.SlipstreamDrive7 + EngineStatsIdealETA.SlipstreamDrive8 + EngineStatsIdealETA.SlipstreamDrive9  + EngineStatsIdealETA.SlipstreamDrive10 + EngineStatsIdealETA.SlipstreamDrive11 + EngineStatsIdealETA.SlipstreamDrive12 + EngineStatsIdealETA.SlipstreamDrive13 + EngineStatsIdealETA.SlipstreamDrive14 + EngineStatsIdealETA.SlipstreamDrive15 + EngineStatsIdealETA.SlipstreamDrive16 + EngineStatsIdealETA.SlipstreamDrive17 + EngineStatsIdealETA.SlipstreamDrive18 + EngineStatsIdealETA.SlipstreamDrive19 + EngineStatsIdealETA.SlipstreamDrive20 
...
def ReturnEngineStats():
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsMenuCalc.SlipstreamDrive1 + EngineStatsMenuCalc.SlipstreamDrive2 + EngineStatsMenuCalc.SlipstreamDrive3 + EngineStatsMenuCalc.SlipstreamDrive4 + EngineStatsMenuCalc.SlipstreamDrive5 + EngineStatsMenuCalc.SlipstreamDrive6 + EngineStatsMenuCalc.SlipstreamDrive7 + EngineStatsMenuCalc.SlipstreamDrive8 + EngineStatsMenuCalc.SlipstreamDrive9  + EngineStatsMenuCalc.SlipstreamDrive10 + EngineStatsMenuCalc.SlipstreamDrive11 + EngineStatsMenuCalc.SlipstreamDrive12 + EngineStatsMenuCalc.SlipstreamDrive13 + EngineStatsMenuCalc.SlipstreamDrive14 + EngineStatsMenuCalc.SlipstreamDrive15 + EngineStatsMenuCalc.SlipstreamDrive16 + EngineStatsMenuCalc.SlipstreamDrive17 + EngineStatsMenuCalc.SlipstreamDrive18 + EngineStatsMenuCalc.SlipstreamDrive19 + EngineStatsMenuCalc.SlipstreamDrive20 
...
def SlipstreamStats(pObject, pEvent):
	...
	# Add the stats of each engine into the list
        ...
        # Can't jump if the engines are below 1%
        if pTotal < 1.0:
            print 'Slipstream: Slipstream Engines are badly damaged, cannot initiate the jump'
            return
...
# Check if the ship is actually equiped with slipstream drive
def HasSlipstream(pShip):        
        # Grab values
	...
	# No ship has 20 engines! Right?!
		...
		if pSystem.GetName() in sSlipstreamList:
                    pStats = pSystem.GetConditionPercentage() * 100.0
                    if pStats >= 1.1:
                        # What's this? Slipstream is installed!
                        return 'Slipstream Installed'
                        break
		pSystem = pShip.GetNextSubsystemMatch(pIterator)
...
def EnteringFlash(pAction):
        ...
        if hasattr(pModule, "SlipstreamCustomizations"):
            ...
            # No sound entry found
                ...
                pEnterSound = App.TGSound_Create("scripts/Custom/Slipstream/SFX/enterslipstream.wav", "Enter", 0)
                pEnterSound.SetSFX(0) 
                pEnterSound.SetInterface(1)
...
def ExitingFlash(pAction):
        ...
        if hasattr(pModule, "SlipstreamCustomizations"):
            ...
            # No sound entry found
            	...
            	pExitSound = App.TGSound_Create("scripts/Custom/Slipstream/SFX/exitslipstream.wav", "Exit", 0)
            	pExitSound.SetSFX(0) 
            	pExitSound.SetInterface(1)
...
def MoveForwardAI(pAction):
        pPlayer = MissionLib.GetPlayer()
        import Custom.Slipstream.Libs.DS9FXWormholeExitingSeqAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Slipstream.Libs.DS9FXWormholeExitingSeqAI.CreateAI(pPlayerCast))
...
def InitiateSearchAI():
        pPlayer = MissionLib.GetPlayer()
        import Custom.Slipstream.Libs.SlipstreamSearchSafePosition
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Slipstream.Libs.SlipstreamSearchSafePosition.CreateAI(pPlayerCast))
...
def ClearAIs(pAction):        
        pPlayer = MissionLib.GetPlayer()        
        import Custom.Slipstream.Libs.DS9FXStayAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Slipstream.Libs.DS9FXStayAI.CreateAI(pPlayerCast))
...
def Engage(pObject, pEvent):
	...
	TacticalInterfaceHandlers.ClearTarget(None, None)
        ...
        if App.g_kSetManager.GetSet("SlipstreamTunnel1"):
            
                pDummySet = App.g_kSetManager.GetSet("SlipstreamTunnel1")

        else:
        
                pDummy = __import__("Systems.SlipstreamTunnel.SlipstreamTunnel1")
        ...
        for kShip in pSet.GetClassObjectList(App.CT_SHIP):
                ...
                # Slipstream ability?!
                SlipstreamInstalled = HasSlipstream(fShip)
                if not SlipstreamInstalled == 'Slipstream Installed':
        ...
        pTunnel = loadspacehelper.CreateShip("slipstream", pDummySet, TunnelString, "Player Start")
        ...
        pTunnel2 = loadspacehelper.CreateShip("slipstream", pDummySet, TunnelString2, "Player Start")
	...
	# Custom tunnel textures?
		...
		fTunnel.RefreshReplacedTextures()
            ...
            # There's no gfx defined, use the user specified defaults
                ...
                if SlipstreamConfiguration.TunnelGFX == 'Default':
                    pass
                else:
                    GFX = "scripts/Custom/Slipstream/GFX/" + SlipstreamConfiguration.TunnelGFX
        ...
        if SlipstreamConfiguration.LoopSound == 1:
            # Play the loop sound
            App.g_kSoundManager.PlaySound("SlipstreamEngineSound")

        if SlipstreamConfiguration.Prints == 1:
        ...
        if hasattr(pModule, "SlipstreamCustomizations"):
            ...
            if pCustomization.has_key('BackdropTexture'):
                ...
                if pCustomization['BackdropTexture'] == "Off":
                    ...
                    Backdrop = "scripts/Custom/Slipstream/GFX/" + pCustomization['BackdropTexture']
            ...
            # There's no gfx defined, use the user specified defaults
                ...
                if SlipstreamConfiguration.BackdropGFX == 'Default':
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
                    ...
                    kUp.SetXYZ(0.049823, 0.254099, 0.965894)
                    kThis.AlignToVectors(kForward, kUp)
                    kThis.SetTextureFileName("data/slipstreamback.tga")
                    kThis.SetTargetPolyCount(256)
                    kThis.SetHorizontalSpan(1.000000)
                    kThis.SetVerticalSpan(1.000000)
                    kThis.SetSphereRadius(300.000000)
                    ...
                    kThis.Update(0)
                    kThis = None
                elif SlipstreamConfiguration.BackdropGFX == 'Off':
                    pass
                else:
                    Backdrop = "scripts/Custom/Slipstream/GFX/" + SlipstreamConfiguration.BackdropGFX
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
...
def AIExit(pObject, pEvent):
        ...
        # Now the magic happens
        pPlayer = MissionLib.GetPlayer()
        pSet = App.g_kSetManager.GetSet('SlipstreamTunnel1')
        ...
        pSet.DeleteObjectFromSet("Slipstream Outer")
        pSet.DeleteObjectFromSet("Slipstream Inner")
		...
		pLocation = pPlayer.GetWorldLocation() 
                ...
                # Is there a customization for this ship available?
                if hasattr(pModules, "SlipstreamCustomizations"):
                    pCustomization = pModules.SlipstreamCustomizations()
...
# When a new object is created in the slipstream set it collides with the massive tunnel thus darkening the screen
def FixDarkScreen(pObject, pEvent):
        pPlayer = App.Game_GetCurrentPlayer()    
        pSet = pPlayer.GetContainingSet()

        if (pSet.GetName() == "SlipstreamTunnel1"):            
            fTunnel = MissionLib.GetShip("Slipstream Outer", pSet) 
            fTunnel2 = MissionLib.GetShip("Slipstream Inner", pSet) 
...
def FixDarkScreenExplosionsDelay(pObject, pEvent):
        ...
        # Create the tunnel
        TunnelString2 = "Slipstream Inner"
        pTunnel2 = loadspacehelper.CreateShip("slipstream", pSet, TunnelString2, "Player Start")
...
def RemoveMenu():
        ...
        pMenu = pHelm.GetMenu()
	if (pMenu != None):
		pButton = pMenu.GetSubmenu("Slipstream Details")
		if (pButton != None):
