...
"""
TMP Warp Startup script

Usage (mostly by example this time, sorry, had to cut it a bit due to school pressure):
Foundation.ShipDef.TMPConstitution.dTechs = {
	# Here comes the TMPWarmUp script config
	...
	"TMPWarpStartUp": {
		"track": {
			"Fca6_glow": {
				...
				0.0: "data/Models/Ships/Constitution Refit/EnterpriseA/High/Engines_glow.tga", 
				1.0: "data/Models/Ships/Constitution Refit/EnterpriseA/High/Engines_glowWarp.tga",
...
class TMPWarpStartUpDef(FoundationTech.TechDef):
	def AttachShip(self, pShip, pInstance):
		...
		if pShip and getattr(pInstance, self.name, None):
			...
			if not getattr(pInstance, "bAddedTMPWarpHandlers", 0):
				...
				pInstance.bAddedTMPWarpHandlers = 1

	def DetachShip(self, pShipID, pInstance):
		pShip = App.ShipClass_Cast(App.TGObject_GetTGObjectPtr(pInstance.pShipID))
		if pShip and getattr(pInstance, "bAddedTMPWarpHandlers", 0):
			...
			pInstance.bAddedTMPWarpHandlers = 0


oTMPWarpStartUp = TMPWarpStartUpDef("TMPWarpStartUp")
...
def StartingWarp(pObject, pEvent):
	...
	dConfig = getattr(pInstance, "TMPWarpStartUp")
	if not dConfig:
		return
