...
import nt
import string
from Custom.Warpspace.Libs import DS9FXMenuLib
from Custom.Warpspace.Libs import LoadFlash
from Custom.Warpspace.Libs import AimForGoodPos
from Custom.Warpspace.Libs import AimForGoodPosExit
from Custom.Warpspace.Libs import EngineStats
from Custom.Warpspace.Libs import EngineStatsETA
from Custom.Warpspace.Libs import EngineStatsIdealETA
from Custom.Warpspace.Libs import EngineStatsMenuCalc
...
# List of subsystem properties Warpspace looks for
sWarpspaceList = ['Warpspace Drive 1', 'Warpspace Drive 2', 'Warpspace Drive 3', 'Warpspace Drive 4', 'Warpspace Drive 5', 'Warpspace Drive 6', 'Warpspace Drive 7', 'Warpspace Drive 8', 'Warpspace Drive 9', 'Warpspace Drive 10', 'Warpspace Drive 11', 'Warpspace Drive 12', 'Warpspace Drive 13', 'Warpspace Drive 14', 'Warpspace Drive 15', 'Warpspace Drive 16', 'Warpspace Drive 17', 'Warpspace Drive 18', 'Warpspace Drive 19', 'Warpspace Drive 20']
...
# Path
sPath = "scripts\\Custom\\Warpspace\\Libs\\EngineStats.py"
sPathETA = "scripts\\Custom\\Warpspace\\Libs\\EngineStatsETA.py"
sPathIdealETA = "scripts\\Custom\\Warpspace\\Libs\\EngineStatsIdealETA.py"
sPathStats = "scripts\\Custom\\Warpspace\\Libs\\EngineStatsMenuCalc.py"

# Tunnel Engine noise is hardcoded and not customizable
pWarpspaceEngineSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/Warpspaceenginenoise.wav", "WarpspaceEngineSound", 0)
pWarpspaceEngineSound.SetSFX(0) 
pWarpspaceEngineSound.SetInterface(1)
pWarpspaceEngineSound.SetLooping(1)


# Main function called by the zzzWarpspaceMode.py
def init():
        ...
        # Navpoint exists?
        try:
            pSet.DeleteObjectFromSet("Warpspace Navpoint")
        except:
            pass
        ...
        WarpspaceInstalled = HasWarpspace(pPlayer)
        if not WarpspaceInstalled == 'Warpspace Installed':
            return

        # Create menus and buttons
        bMenu = DS9FXMenuLib.CreateMenu("Warpspace Details", "Helm", 2, 0)
        bButton = DS9FXMenuLib.CreateButton("Warpspace Drive", "Helm", __name__ + ".WarpspaceGUI", "Warpspace Details")
        bInterceptMenu = App.STMenu_CreateW(App.TGString("Intercept Options"))
	bMenu.PrependChild(bInterceptMenu)
        bInterceptButton = DS9FXMenuLib.CreateBridgeMenuButton("Warpspace Intercept", "Helm", __name__ + ".WarpspaceInterceptStats", bInterceptMenu)
        bNavPointButton = DS9FXMenuLib.CreateBridgeMenuButton("Initiate Navpoint Mode", "Helm", __name__ + ".WarpspaceNavpoint", bInterceptMenu)
	...
	bNavPointButton.SetNormalColor(NormalColor2)
        ...
        # Resolve the small issue when the warp button is enabled and Warpspace is not
...
def handlers():
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        if pMission.GetScript() == "QuickBattle.QuickBattle":
            print 'Warpspace: Quick Battle is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
        elif pMission.GetScript() == "Custom.QuickBattleGame.QuickBattle":
            print 'Warpspace: Quick Battle Replacement is running... Mod is starting up...'
            pSupported = 1
            # Possible bugfix
            init()
	else:
	    print 'Warpspace: Unsupported game type is running... Mod is shutting down...'
	    pSupported = 0
	    return
...
def quitting():
        ...
        try:
            App.g_kSoundManager.StopSound("WarpspaceEngineSound")
        except:
            pass
        try:
        ...
        try:
            ...
            pass
        try:
            App.g_kEventManager.RemoveBroadcastHandler(ET_ENGAGE, pMission, __name__ + ".WarpspaceStats")
        except:
            pass
        try:
...
def CourseSet(pObject, pEvent):
        ...
        if not (pEvent.GetInt() == App.CharacterClass.EST_SET_COURSE_INTERCEPT):
            ...
            # Incompatibility fix for hyperdrive & warpspace
...
# Create a navpoint
def WarpspaceNavpoint(pObject, pEvent):
...
def MoveNavpoint(pObject, pEvent):
        ...
        fNav = MissionLib.GetShip("Warpspace Navpoint", pSet) 
        fNav.SetTranslateXYZ(pLoc.GetX() + cordX, pLoc.GetY() + cordY, pLoc.GetZ() + cordZ)
        fNav.UpdateNodeOnly()


# The newest addition to the warpspace family in system warpspace intercept, fast as hell on great distances
def WarpspaceInterceptStats(pObject, pEvent):
        global lStats
	...
	# Add the stats of each engine into the list
        while (pSystem != None):
		if pSystem.GetName() in sWarpspaceList:
		...
		for i in range(pSystem.GetNumChildSubsystems()):
			...
			if pChild.GetName() in sWarpspaceList:
				pStats = pSystem.GetConditionPercentage() * 100.0
				pDisabled = pSystem.GetDisabledPercentage() * 100.0
				if pStats <= pDisabled:
					# Engine is disabled, treat it like it's destroyed
        ...
        nt.write(file, "# Do not Edit this file, Warpspace uses it to store data\n")
        i = 0
        # Write stats into the file
        for stat in lStats:
            i = i + 1
            nt.write(file, "\nWarpspaceDrive" + str(i) + " = " + str(stat))
        ...
        iTotal = EngineStats.WarpspaceDrive1 + EngineStats.WarpspaceDrive2 + EngineStats.WarpspaceDrive3 + EngineStats.WarpspaceDrive4 + EngineStats.WarpspaceDrive5 + EngineStats.WarpspaceDrive6 + EngineStats.WarpspaceDrive7 + EngineStats.WarpspaceDrive8 + EngineStats.WarpspaceDrive9  + EngineStats.WarpspaceDrive10 + EngineStats.WarpspaceDrive11 + EngineStats.WarpspaceDrive12 + EngineStats.WarpspaceDrive13 + EngineStats.WarpspaceDrive14 + EngineStats.WarpspaceDrive15 + EngineStats.WarpspaceDrive16 + EngineStats.WarpspaceDrive17 + EngineStats.WarpspaceDrive18 + EngineStats.WarpspaceDrive19 + EngineStats.WarpspaceDrive20 

        pTotal = iTotal / nTotal

        # Can't warp if the engines are below 1%
        if pTotal < 1.0:
            print 'Warpspace: Warpspace Engines are badly damaged, cannot intercept target'
            return
...
def InterceptTargetCheck():
        ...
        if pTargetPlanet:
            if not DistanceCheck(pTargetPlanet) >= 400:
                ...
                pSequence.Play()
                # print 'Warpspace: Planet is too close...'
                return
            ...
            if not DistanceCheck(pTargetShip) >= 250:
                ...
                pSequence.AddAction(pAction, None, 1)
                pSequence.Play()
                # print 'Warpspace: Ship is too close...'
                return
...
def ResetButtonString(pAction, pButton):
        try:
            pButton.SetName(App.TGString("Warpspace Intercept"))
        except:
            pass
...
def InterceptTarget(pTarget, IsPlanet):
        ...
        pWasCloaked = 0
        if pPlayer.IsCloaked() and not (HasPhasedWarpspace(pPlayer) == "Phased Warpspace Installed"):
            pWasCloaked = 1
...
def AdditionalPosAndCheckIntercept(pAction, pTarget, IsPlanet):
        ...
        if sAim == 'TRUE':               
                ...
                pEntro = "WarpspaceFlash"
                pSalgo = "WarpspaceFlashExit"
                pSonidoEntro = "EnteringFlash"
                pSonidoSalgo = "ExitingFlash"
                if (HasSigmaWarpspace(pPlayer) == "Sigma Walkers Warpspace Installed"):
                    pEntro = "SigmaFlash"
                    pSalgo = "SigmaFlash"
                if (HasZergWarpspace(pPlayer) == "Zerg Warpspace Installed"):
                    pEntro = "ZergFlash"
                    pSalgo = "ZergFlash"
                if (HasPhasedWarpspace(pPlayer) == "Phased Warpspace Installed"):
...
def InaccionEntry():
        pEnterSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/dummy.wav", "Enter", 0)
...
def InaccionExit():
        pEnterSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/dummy.wav", "Exit", 0)
        pEnterSound.SetSFX(0) 
        pEnterSound.SetInterface(1)
...
# Reset navpoints after in system warp
def ResetNavpoints():
...
def WarpspaceGUI(pObject, pEvent):
	...
	pMission = pEpisode.GetCurrentMission()
        ...
        # Setup Events
	App.g_kEventManager.AddBroadcastPythonFuncHandler(ET_ENGAGE, pMission, __name__ + ".WarpspaceStats")
...
def CreateEntries(pObject, pEvent):
        ...
        pOptionsWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Engage Warpspace"), 0.0, 0.0, None, 1, 0.19, 0.19, App.g_kMainMenuBorderMainColor)
...
def ReturnETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsETA.WarpspaceDrive1 + EngineStatsETA.WarpspaceDrive2 + EngineStatsETA.WarpspaceDrive3 + EngineStatsETA.WarpspaceDrive4 + EngineStatsETA.WarpspaceDrive5 + EngineStatsETA.WarpspaceDrive6 + EngineStatsETA.WarpspaceDrive7 + EngineStatsETA.WarpspaceDrive8 + EngineStatsETA.WarpspaceDrive9  + EngineStatsETA.WarpspaceDrive10 + EngineStatsETA.WarpspaceDrive11 + EngineStatsETA.WarpspaceDrive12 + EngineStatsETA.WarpspaceDrive13 + EngineStatsETA.WarpspaceDrive14 + EngineStatsETA.WarpspaceDrive15 + EngineStatsETA.WarpspaceDrive16 + EngineStatsETA.WarpspaceDrive17 + EngineStatsETA.WarpspaceDrive18 + EngineStatsETA.WarpspaceDrive19 + EngineStatsETA.WarpspaceDrive20 

        pTotal = iTotal / nTotal

        # Can't warp if the engines are below 1%, return Engines Disabled ETA
        if pTotal < 1.0:
            return ' '
...
def ReturnIdealETA(pSelectedSpeed):
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsIdealETA.WarpspaceDrive1 + EngineStatsIdealETA.WarpspaceDrive2 + EngineStatsIdealETA.WarpspaceDrive3 + EngineStatsIdealETA.WarpspaceDrive4 + EngineStatsIdealETA.WarpspaceDrive5 + EngineStatsIdealETA.WarpspaceDrive6 + EngineStatsIdealETA.WarpspaceDrive7 + EngineStatsIdealETA.WarpspaceDrive8 + EngineStatsIdealETA.WarpspaceDrive9  + EngineStatsIdealETA.WarpspaceDrive10 + EngineStatsIdealETA.WarpspaceDrive11 + EngineStatsIdealETA.WarpspaceDrive12 + EngineStatsIdealETA.WarpspaceDrive13 + EngineStatsIdealETA.WarpspaceDrive14 + EngineStatsIdealETA.WarpspaceDrive15 + EngineStatsIdealETA.WarpspaceDrive16 + EngineStatsIdealETA.WarpspaceDrive17 + EngineStatsIdealETA.WarpspaceDrive18 + EngineStatsIdealETA.WarpspaceDrive19 + EngineStatsIdealETA.WarpspaceDrive20 
...
def ReturnEngineStats():
	...
	# Add the stats of each engine into the list
        ...
        iTotal = EngineStatsMenuCalc.WarpspaceDrive1 + EngineStatsMenuCalc.WarpspaceDrive2 + EngineStatsMenuCalc.WarpspaceDrive3 + EngineStatsMenuCalc.WarpspaceDrive4 + EngineStatsMenuCalc.WarpspaceDrive5 + EngineStatsMenuCalc.WarpspaceDrive6 + EngineStatsMenuCalc.WarpspaceDrive7 + EngineStatsMenuCalc.WarpspaceDrive8 + EngineStatsMenuCalc.WarpspaceDrive9  + EngineStatsMenuCalc.WarpspaceDrive10 + EngineStatsMenuCalc.WarpspaceDrive11 + EngineStatsMenuCalc.WarpspaceDrive12 + EngineStatsMenuCalc.WarpspaceDrive13 + EngineStatsMenuCalc.WarpspaceDrive14 + EngineStatsMenuCalc.WarpspaceDrive15 + EngineStatsMenuCalc.WarpspaceDrive16 + EngineStatsMenuCalc.WarpspaceDrive17 + EngineStatsMenuCalc.WarpspaceDrive18 + EngineStatsMenuCalc.WarpspaceDrive19 + EngineStatsMenuCalc.WarpspaceDrive20 
...
def WarpspaceStats(pObject, pEvent):
	...
	# Add the stats of each engine into the list
        ...
        # Can't warp if the engines are below 1%
        if pTotal < 1.0:
            print 'Warpspace: Warpspace Engines are badly damaged, cannot initiate the warp'
            return
...
# Check if the ship is actually equiped with warpspace drive
def HasWarpspace(pShip):        
	# Grab values
	...
	while (pSystem != None):
		if pSystem.GetName() in sWarpspaceList:
			pStats = pSystem.GetConditionPercentage()
			if pStats >= pSystem.GetDisabledPercentage():
				# What's this? Warpspace is installed!
				return 'Warpspace Installed'
				break
		for i in range(pSystem.GetNumChildSubsystems()):
			pChild = pSystem.GetChildSubsystem(i)
			if pChild.GetName() in sWarpspaceList:
	...
	return 'Warpspace not Installed'

# Checks if this ship can do phased Warpspace
def HasPhasedWarpspace(pPlayer):
        pIterator = pPlayer.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
	pSystem = pPlayer.GetNextSubsystemMatch(pIterator)
	...
	# Add the stats of each engine into the list
		...
		if pSystem.GetName() == "Hyperspace Cloak":
                   ...
                   if pStats > pDisabled:
                       ...
                       return 'Phased Warpspace Installed'
                       break                
		pSystem = pPlayer.GetNextSubsystemMatch(pIterator)
...
def HasSigmaWarpspace(pPlayer):
	...
	# Add the stats of each engine into the list
		...
		if pSystem.GetName() == "TransDimensional Drive":
                   ...
                   if pStats > pDisabled:
                       # Engine is enabled, means it works
                       return 'Sigma Walkers Warpspace Installed'
                       break                
		pSystem = pPlayer.GetNextSubsystemMatch(pIterator)

        pPlayer.EndGetSubsystemMatch(pIterator)

# Checks if this ship can do Zerg Wormholes
def HasZergWarpspace(pPlayer):
        pIterator = pPlayer.StartGetSubsystemMatch(App.CT_SHIP_SUBSYSTEM)
	pSystem = pPlayer.GetNextSubsystemMatch(pIterator)

	# Add the stats of each engine into the list
        while (pSystem != None):
		if pSystem.GetName() == "Zerg Wormhole Drive":
                   pStats = pSystem.GetConditionPercentage() * 100.0
                   pDisabled = pSystem.GetDisabledPercentage() * 100.0
                   if pStats > pDisabled:
                       # Engine is enabled, means it works
                       return 'Zerg Warpspace Installed'
                       break                
		pSystem = pPlayer.GetNextSubsystemMatch(pIterator)

        pPlayer.EndGetSubsystemMatch(pIterator)

# Warpspace sequence initiated
...
def EnteringFlash(pAction):
        ...
        if hasattr(pModule, "WarpspaceCustomizations"):
            ...
            if pCustomization.has_key('EntrySound'):
                pSound = "scripts/Custom/Warpspace/SFX/" + pCustomization['EntrySound']
            ...
            # No sound entry found
                ...
                pEnterSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/enterwarpspace.wav", "Enter", 0)
                pEnterSound.SetSFX(0) 
                pEnterSound.SetInterface(1)
...
def ShadowFlashEntry(pAction):
        pEnterSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/shadowPhasedScream.wav", "Enter", 0)
        pEnterSound.SetSFX(0) 
        pEnterSound.SetInterface(1)
...
def ShadowFlashExit(pAction):
        pEnterSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/shadowPhasedScream.wav", "Exit", 0)
        pEnterSound.SetSFX(0) 
        pEnterSound.SetInterface(1)
...
def ExitingFlash(pAction):
        ...
        if hasattr(pModule, "WarpspaceCustomizations"):
            ...
            if pCustomization.has_key('ExitSound'):
                pSound = "scripts/Custom/Warpspace/SFX/" + pCustomization['ExitSound']
            ...
            # No sound entry found
            	...
            	pExitSound = App.TGSound_Create("scripts/Custom/Warpspace/SFX/exitwarpspace.wav", "Exit", 0)
            	pExitSound.SetSFX(0) 
            	pExitSound.SetInterface(1)
...
def MoveForwardAI(pAction):
        pPlayer = MissionLib.GetPlayer()
        import Custom.Warpspace.Libs.DS9FXWormholeExitingSeqAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Warpspace.Libs.DS9FXWormholeExitingSeqAI.CreateAI(pPlayerCast))
...
def InitiateSearchAI():
        pPlayer = MissionLib.GetPlayer()
        import Custom.Warpspace.Libs.WarpspaceSearchSafePosition
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Warpspace.Libs.WarpspaceSearchSafePosition.CreateAI(pPlayerCast))
...
def ClearAIs(pAction):        
        pPlayer = MissionLib.GetPlayer()        
        import Custom.Warpspace.Libs.DS9FXStayAI
        pPlayerCast = App.ShipClass_Cast(pPlayer)
        pPlayerCast.SetAI(Custom.Warpspace.Libs.DS9FXStayAI.CreateAI(pPlayerCast))
...
def BackToNormal(pAction):
	...
	# Restore things back to normal
        ...
        if pWasCloaked == 1 and not (HasPhasedWarpspace(pPlayer) == "Phased Warpspace Installed"):
            pCloak = pPlayer.GetCloakingSubsystem()
            if pCloak:
                pCloak.InstantCloak()
...
def WarpspaceFlash(pAction):
        ...
        return 0
def WarpspaceFlashExit(pAction):
        # print 'Warpspace exit should be starting now...'    
        pPlayer = MissionLib.GetPlayer()
...
def ZergFlash(pAction):
            
        pPlayer = MissionLib.GetPlayer()
            
        # Load texture GFX
        LoadFlash.ZergFlashGFX()
        # Create flash
        LoadFlash.ZergCreateGFX(pPlayer)
...
def Engage(pObject, pEvent):
        ...
        # Resolve the dark screen problem after a new object is created in the warpspace\hyperdrive set
        pGame = App.Game_GetCurrentGame()
	pEpisode = pGame.GetCurrentEpisode()
	pMission = pEpisode.GetCurrentMission()
	...
	TacticalInterfaceHandlers.ClearTarget(None, None)
        ...
        if App.g_kSetManager.GetSet("WarpspaceTunnel1"):
            
                pDummySet = App.g_kSetManager.GetSet("WarpspaceTunnel1")

        else:
        
                pDummy = __import__("Systems.WarpspaceTunnel.WarpspaceTunnel1")
        ...
        for kShip in pSet.GetClassObjectList(App.CT_SHIP):
                ...
                # Warpspace ability?!
                WarpspaceInstalled = HasWarpspace(fShip)
                if not WarpspaceInstalled == 'Warpspace Installed':
        ...
        # Create the tunnel
        TunnelString = "Warpspace Outer"
        pTunnel = loadspacehelper.CreateShip("warpspace", pDummySet, TunnelString, "Player Start")
        ...
        # Create the tunnel
        TunnelString2 = "Warpspace Inner"
        pTunnel2 = loadspacehelper.CreateShip("warpspace", pDummySet, TunnelString2, "Player Start")
	...
	# Custom tunnel textures?
        ...
        if hasattr(pModule, "WarpspaceCustomizations"):
            ...
            if pCustomization.has_key('TunnelTexture'):
                ...
                GFX = "scripts/Custom/Warpspace/GFX/" + pCustomization['TunnelTexture']
		...
		fTunnel.RefreshReplacedTextures()
            ...
            # There's no gfx defined, use the user specified defaults
                ...
                if WarpspaceConfiguration.TunnelGFX == 'Default':
                    pass
                else:
                    GFX = "scripts/Custom/Warpspace/GFX/" + WarpspaceConfiguration.TunnelGFX
        ...
        if WarpspaceConfiguration.LoopSound == 1:
            # Play the loop sound
            App.g_kSoundManager.PlaySound("WarpspaceEngineSound")

        if WarpspaceConfiguration.Prints == 1:
        ...
        if hasattr(pModule, "WarpspaceCustomizations"):
            ...
            if pCustomization.has_key('BackdropTexture'):
                ...
                if pCustomization['BackdropTexture'] == "Off":
                    ...
                    Backdrop = "scripts/Custom/Warpspace/GFX/" + pCustomization['BackdropTexture']
            ...
            # There's no gfx defined, use the user specified defaults
                ...
                if WarpspaceConfiguration.BackdropGFX == 'Default':
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
                    ...
                    kUp.SetXYZ(0.049823, 0.254099, 0.965894)
                    kThis.AlignToVectors(kForward, kUp)
                    kThis.SetTextureFileName("data/warpspaceback.tga")
                    kThis.SetTargetPolyCount(256)
                    kThis.SetHorizontalSpan(1.000000)
                    kThis.SetVerticalSpan(1.000000)
                    kThis.SetSphereRadius(300.000000)
                    ...
                    kThis.Update(0)
                    kThis = None
                elif WarpspaceConfiguration.BackdropGFX == 'Off':
                    pass
                else:
                    Backdrop = "scripts/Custom/Warpspace/GFX/" + WarpspaceConfiguration.BackdropGFX
                    kThis = App.StarSphere_Create()
                    kThis.SetName("Backdrop stars")
                    kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
                    kForward = App.TGPoint3()
...
def AIExit(pObject, pEvent):
        ...
        # Now the magic happens
        pPlayer = MissionLib.GetPlayer()
        pSet = App.g_kSetManager.GetSet('WarpspaceTunnel1')
        ...
        pSet.DeleteObjectFromSet("Warpspace Outer")
        pSet.DeleteObjectFromSet("Warpspace Inner")
		...
		pLocation = pPlayer.GetWorldLocation() 
                ...
                # Is there a customization for this ship available?
                if hasattr(pModules, "WarpspaceCustomizations"):
                    pCustomization = pModules.WarpspaceCustomizations()
...
# When a new object is created in the warpspace set it collides with the massive tunnel thus darkening the screen
def FixDarkScreen(pObject, pEvent):
        pPlayer = App.Game_GetCurrentPlayer()    
        pSet = pPlayer.GetContainingSet()

        if (pSet.GetName() == "WarpspaceTunnel1"):            
            fTunnel = MissionLib.GetShip("Warpspace Outer", pSet) 
            fTunnel2 = MissionLib.GetShip("Warpspace Inner", pSet) 
...
def FixDarkScreenExplosionsDelay(pObject, pEvent):
        ...
        # Create the tunnel
        TunnelString = "Warpspace Outer"
        pTunnel = loadspacehelper.CreateShip("warpspace", pSet, TunnelString, "Player Start")
        ...
        # Create the tunnel
        TunnelString2 = "Warpspace Inner"
        pTunnel2 = loadspacehelper.CreateShip("warpspace", pSet, TunnelString2, "Player Start")
...
def RemoveMenu():
        ...
        pMenu = pHelm.GetMenu()
	if (pMenu != None):
		pButton = pMenu.GetSubmenu("Warpspace Details")
		if (pButton != None):
