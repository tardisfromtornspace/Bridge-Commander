...
##########	MANUAL
#################################################################################################################
# NOTE: all functions/methods and attributes defined here (in this prototype example plugin, B5Quantumspace) are required to be in the plugin, with the exclusion of:
...
# ------ Auxiliar functions: "AnObjectDying", "AuxProtoElementNames", "B5QuantumspaceFlash", "CreateDetachedElectricExplosion", "findShipInstance", "HasPhasedQuantumspace", "HasSigmaQuantumspace", "LoadGFX", "PlayB5QuantumspaceSound", "PlayB5QuantumspaceSoundC", "B5QuantumspaceBasicConfigInfo" and "B5QuantumspaceDisabledCalculations".
# ------ Auxiliar variables: "defineTravelSpaceNoise", "myGlobalAISet", "myGlobalpSet" and "pQuantumspaceEngineSound", used for keeping tabs on our alternate pSets and the sound that plays while on jumpspace.
# ------ Auxiliar class "WhyMissionLibGetsError" and its instance "basicListener"
# ------ Auxiliar functions for intra-system intercept (ISI) support, which as a result of being a common-made function between default GalaxyCharts functions/methods, regular AlternateSubModelFTL and ISI, while not required to be on the plugin, some of their contents are actually required if they are not there: "awayNavPointDistanceCalc", "CanTravelShip", "ConditionalCloak", "ConditionalDecloak", "EngageSeqTractorCheckI", "GetEngageDirectionISI", "GetExitedTravelEventsI", "GetStartTravelEventsI", "GetEngageDirectionC", "InSystemIntercept", "MaintainTowingActionI", "PlayB5QuantumspaceSoundI", "removeTractorISITowInfo", "SetupSequenceISI" and "SetupTowingI".
...
# On this case, due to that, only the lines marked with "# (#)" are needed for B5Quantumspace to work, but the final parent technology may require more:
# "B5Quantumspace": is the name of the key. This is the bare minimum for the technology to work
# "Nacelles": is the name of a key whose value indicates a list of which warp engine property children (nacelles) are part of the B5Quantumspace system. If all are disabled/destroyed, B5Quantumspace will not engage. If this field does not exist or "Nacelles": [] it skips this disabled check.
# "Core": is the name of a key whose value indicates a list of which hardpoint properties (not nacelles) are part of the B5Quantumspace system. If all are disabled/destroyed, Quantumspace will not engage either. If this field does not exist, it wil check for all subsystems with "quantum jumpspace drive" or "quantum jump-space drive", case insensitive. Use "Core": [] to skip this check.
...
Foundation.ShipDef.EAOmega.dTechs = { # (#)
	"Alternate-Warp-FTL": { # (#)
		...
		"Port Wing":     ["VasKholhr_Portwing", {
			...
			"WarpDuration":       150.0,
			"B5QuantumspaceRotation":       [0, 0.749, 0],
			"B5QuantumspacePosition":       [0, 0, 0.05],
			"B5QuantumspaceDuration":       150.0,
			},
		],
...
########
sGoingTo = "Entering Quantumspace to go to"

########
# Phrase to show when the ship drops out of travel (while travelling)
########
sDropOut = "Dropped out of Quantumspace..."
...
def KindOfMove(): 
	return "B5Quantumspace" # Modify this with the name you are gonna use for the AlternateSubModelFTL
...
def IsShipEquipped(pShip):
	...
	if pInstance:
		if pInstanceDict.has_key("Alternate-Warp-FTL") and pInstanceDict["Alternate-Warp-FTL"].has_key("Setup") and pInstanceDict["Alternate-Warp-FTL"]["Setup"].has_key("B5Quantumspace"): # You need to add a foundation technology to this vessel "AlternateSubModelFTL"
			return 1
...
def HasPhasedQuantumspace(pShip, myStr = "Hyperspace Cloak"):
	found = 0
...
def HasSigmaQuantumspace(pShip):
	return HasPhasedQuantumspace(pShip, myStr = "TransDimensional Drive")
...
def AuxProtoElementNames(*args):
	# Returns hardpoint property name fragments that indicate are part of the B5Quantumspace system, and blacklists
	return ["quantum jumpspace drive", "quantum jump-space drive"], ["not a quantum jumpspace drive", "not a quantum jump-space drive", " not quantum jumpspace drive", " not quantum jump-space drive"]

# This is just another auxiliar function I made for this
def B5QuantumspaceBasicConfigInfo(pShip):
	...
	if pInstance:
		...
		if pInstancedict.has_key("Alternate-Warp-FTL") and pInstancedict["Alternate-Warp-FTL"].has_key("Setup") and pInstancedict["Alternate-Warp-FTL"]["Setup"].has_key("B5Quantumspace"):
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5Quantumspace"].has_key("Nacelles"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificNacelleHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5Quantumspace"]["Nacelles"]
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5Quantumspace"].has_key("Core"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificCoreHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5Quantumspace"]["Core"]
...
def B5QuantumspaceDisabledCalculations(type, specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pSubsystem, pShip, justFindOne=0):
	...
	return totalB5QuantumspaceEngines, onlineB5QuantumspaceEngines
...
def CanTravelShip(pShip):
	...
	if not isEquipped:
		...
		return "This ship is not equipped with B5Quantumspace"
			...
			if bIsPlayer == 1:
				if pHelm:
					...
					if pDatabase:
						...
						pSequence.Play ()
						App.g_kLocalizationManager.Unload (pDatabase)
			return "Quantumspace Drive is disabled"
...
def CanContinueTravelling(self):
	...
	if not isEquipped:
		if bIsPlayer == 1:
			pSequence = App.TGSequence_Create ()
			pSubtitleAction = App.SubtitleAction_CreateC("Brex: We don't have B5Quantumspace sir, we are dropping out of it.")
			pSubtitleAction.SetDuration(3.0)
			pSequence.AddAction(pSubtitleAction)
			pSequence.Play()
		bStatus = 0
		...
		if totalB5QuantumspaceCores > 0 and onlineB5QuantumspaceCores <= 0:
			if bIsPlayer == 1:
				pSequence = App.TGSequence_Create ()
				pSubtitleAction = App.SubtitleAction_CreateC("Brex: All Quantumspace systems are offline or disabled sir, however I've managed to get us out of Quantumspace before the systems failed utterly.")
				pSubtitleAction.SetDuration(3.0)
				pSequence.AddAction(pSubtitleAction)
				pSequence.Play()
			bStatus = 0
			...
			if totalB5QuantumspaceEngines > 0 and onlineB5QuantumspaceEngines <= 0:
				if bIsPlayer == 1:
					pSequence = App.TGSequence_Create ()
					pSubtitleAction = App.SubtitleAction_CreateC("Brex: All Quantumspace vortex generators are now offline or disabled sir, however I've managed to get us out of Quantumspace before the systems failed utterly")
					pSubtitleAction.SetDuration(3.0)
					pSequence.AddAction(pSubtitleAction)
					pSequence.Play()
				bStatus = 0
...
def PlayB5QuantumspaceSound(pAction, pWS, sType, sRace):
	debug(__name__ + ", PlayB5QuantumspaceSound")

	pShip = pWS.GetShip()
	return PlayB5QuantumspaceSoundC(pAction, pShip, sType, sRace)

# Aux. ISI function
def PlayB5QuantumspaceSoundI(pAction, pShipID, sType, sRace):
	debug(__name__ + ", PlayB5QuantumspaceSoundI")
	pShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pShipID)
	if pShip != None:
		PlayB5QuantumspaceSoundC(pAction, pShip, sType, sRace)
	return 0

# Aux. ISI function
def PlayB5QuantumspaceSoundC(pAction, pShip, sType, sRace):
	debug(__name__ + ", PlayB5QuantumspaceSoundC")
	...
	if pSet.GetRegionModule() == pPlaSet.GetRegionModule():
		try:
			...
			elif HasPhasedQuantumspace(pShip):
				if sType == "Enter Warp":
			...
			try:
				...
				print "B5Quantumspace TravellingMethod: error while calling PlayB5QuantumspaceSoundC:"
				traceback.print_exc()
	return 0
...
def ConditionalDecloak(pAction, pShipID, instant = 0):
	...
	if HasPhasedQuantumspace(pShip):
		if pShip.IsCloaked():
...
def CreateDetachedElectricExplosion(fRed, fGreen, fBlue, fSize, fLifeTime, sFile, pEmitFrom, pAttachTo, fFrequency=1,fEmitLife=1,fSpeed=1.0):
	...
	try:     
		...
		print "B5Quantumspace TravellingMethod: error while calling CreateDetachedElectricExplosion:"
		traceback.print_exc()
		pEffect = None
...
def B5QuantumspaceFlash(pAction, pShipID, sType, sRace, sparkSize=5, sFile = 'scripts/Custom/TravellingMethods/GFX/QuantumspaceFlashAlternate.tga'):
	pShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pShipID)
	if not pShip:
		return 0
	if pShip == None:
	...
	try:
		...
		elif HasSigmaQuantumspace(pShip):
			if sType == "Enter Warp":
				sFile = "scripts/Custom/TravellingMethods/GFX/SigmaQuantumspaceFlash.tga"
			else:
				sFile = "scripts/Custom/TravellingMethods/GFX/SigmaQuantumspaceFlash.tga"

		else:
			if sType == "Enter Warp":
				sFile = "scripts/Custom/TravellingMethods/GFX/QuantumspaceFlashAlternate.tga"
			else:
				sFile = "scripts/Custom/TravellingMethods/GFX/QuantumspaceFlash.tga"
		...
		if sparkSize > 0:
			...
			try:
				...
				print "B5Quantumspace TravellingMethod: error while calling B5QuantumspaceEnterFlash:"
				traceback.print_exc()
...
def removeTractorISITowInfo(pShipID, pInstance=None):
	...
	if pInstance:
		try:
			if hasattr(pInstance, "BSQuantumspaceISIvTowPosition"):
				del pInstance.BSQuantumspaceISIvTowPosition
		except:
			print "BSQuantumspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "BSQuantumspaceISITowee"):
				del pInstance.BSQuantumspaceISITowee
		except:
			print "BSQuantumspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "BSQuantumspaceISIbTractorStat"):
				del pInstance.BSQuantumspaceISIbTractorStat
		except:
			print "BSQuantumspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
...
def SetupTowingI(ShipID):
	...
	try:
		...
		pInstance.BSQuantumspaceISIvTowPosition = pTarget.GetWorldLocation()
		pInstance.BSQuantumspaceISIvTowPosition.Subtract( pShip.GetWorldLocation() )
		pInstance.BSQuantumspaceISIvTowPosition.MultMatrixLeft( pShip.GetWorldRotation().Transpose() )

		pInstance.BSQuantumspaceISITowee = pTarget.GetObjID()

		pInstance.BSQuantumspaceISIbTractorStat = 1

	except:
		print "BSQuantumspace: Error while calling SetupTowingI"
		traceback.print_exc()
...
def MaintainTowingActionI(pAction, pShipID):
	...
	if not pInstance:
		return 0
	try:
		if hasattr(pInstance, "BSQuantumspaceISIbTractorStat") and pInstance.BSQuantumspaceISIbTractorStat == 1:
			pTractor = pShip.GetTractorBeamSystem()
			if pTractor == None:
			...
			if not hasattr(pInstance, "BSQuantumspaceISITowee"):
				return 0

			targetID = pInstance.BSQuantumspaceISITowee
			...
			vPosition.Set( pInstance.BSQuantumspaceISIvTowPosition )
		...
		print "BSQuantumspace: Error while calling MaintainTowingActionI"
		traceback.print_exc()
...
def EngageSeqTractorCheckI(pAction, pShipID):
	...
	if hasattr(pInstance, "BSQuantumspaceISIbTractorStat") and pInstance.BSQuantumspaceISIbTractorStat == 1 and hasattr(pInstance, "BSQuantumspaceISITowee") and pInstance.BSQuantumspaceISITowee != None:
		# hide the towee

		pToweeShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pInstance.BSQuantumspaceISITowee)
		if pToweeShip:
	...
	if pPlayer and hasattr(pPlayer, "GetObjID"):
		...
		if pPlayerID != None and pPlayerID != App.NULL_ID and pPlayerID == pShipID:
			try:
				...
				print "Error while shutting up the global B5 Quantumspace noise:"
				traceback.print_exc()
...
def defineTravelSpaceNoise(pAction, engage=1):
	...
	global pQuantumspaceEngineSound
	if pQuantumspaceEngineSound == None:
		pQuantumspaceEngineSoundAux = None
		try:
			pQuantumspaceEngineSoundAux = App.TGSound_Create("scripts/Custom/TravellingMethods/SFX/JumpspaceSpaceNoise.wav", "Babylon5QuantumspaceStormSound", 0)
			pQuantumspaceEngineSoundAux.SetSFX(0) 
			pQuantumspaceEngineSoundAux.SetInterface(1)
			pQuantumspaceEngineSoundAux.SetLooping(1)
		except:
			pQuantumspaceEngineSoundAux = None
			print "Error on ", __name__, " defineTravelSpaceNoise:"
			traceback.print_exc()

		if pQuantumspaceEngineSoundAux != None:
			pQuantumspaceEngineSound = pQuantumspaceEngineSoundAux
	...
	if engage == 0:
		try:
			App.g_kSoundManager.StopSound("Babylon5QuantumspaceStormSound")
		...
		try:
			App.g_kSoundManager.PlaySound("Babylon5QuantumspaceStormSound")
...
def SetupSequenceISI(pShip=None):
	...
	hasTractorReady = pInstance != None and hasattr(pInstance, "BSQuantumspaceISIbTractorStat") and (pInstance.BSQuantumspaceISIbTractorStat == 1)
	...
	pWarpSoundAction1 = App.TGScriptAction_Create(__name__, "PlayB5QuantumspaceSoundI", pShipID, "Enter Warp", sRace)
	...
	pFlashAction1 = App.TGScriptAction_Create(__name__, "B5QuantumspaceFlash", pShipID, "Enter Warp", sRace)
	...
	# Check for towee
	if hasTractorReady == 1:
		pHideTowee = App.TGScriptAction_Create(sCustomActionsScript, "HideShip", pInstance.BSQuantumspaceISITowee, 1)
		pExitWarpSeq.AddAction(pHideTowee, pHideShip)

	# Create the warp flash.
	pFlashAction2 = App.TGScriptAction_Create(__name__, "B5QuantumspaceFlash", pShipID, "Exit Warp", sRace)
	...
	if hasTractorReady == 1:
		pUnHideTowee = App.TGScriptAction_Create(sCustomActionsScript, "HideShip", pInstance.BSQuantumspaceISITowee, 0)
	...
	pWarpSoundAction2 = App.TGScriptAction_Create(__name__, "PlayB5QuantumspaceSoundI", pShipID, "Exit Warp", sRace)
...
def SetupSequence(self):
	...
	#	   tractor ships thru your travel. (in this example, thru Quantumspace)
	################################################################################
	...
	if (pPlayer != None) and (pShipID == pPlayer.GetObjID()):
		...
		pWarpSoundAction0 = App.TGScriptAction_Create(__name__, "PlayB5QuantumspaceSoundI", pShipID, "Enter Warp", sRace)
...
def GetTravelSetToUse(self, manual=0):
	...
	try:
		...
		import Custom.GalaxyCharts.TravelerSystems.QuantumspaceTunnelTravelSet
		import Custom.GalaxyCharts.TravelerSystems.AIQuantumspaceTunnelTravelSet
		pSet = None
		if manual == 1:
			if myGlobalpSet == None:
				myGlobalpSet = Custom.GalaxyCharts.TravelerSystems.QuantumspaceTunnelTravelSet.Initialize()
			if myGlobalAISet == None:
				myGlobalAISet = Custom.GalaxyCharts.TravelerSystems.AIQuantumspaceTunnelTravelSet.Initialize()	
		if self.IsPlayer == 1:
			if myGlobalpSet == None:
				try:
					myGlobalpSet = Custom.GalaxyCharts.TravelerSystems.QuantumspaceTunnelTravelSet.Initialize()
					self.Logger.LogString(" -Initialized Quantumspace Tunnel Travel Set")
				except:
					print "ERROR on GetTravelSetToUse: "
					traceback.print_exc()
				#self.Logger.LogString(" -Initialized Quantumspace Tunnel Travel Set")
			pSet = myGlobalpSet #Custom.GalaxyCharts.Traveler.Travel.pQuantumspaceTunnelTravelSet
			...
			if myGlobalAISet == None:
				try:
					myGlobalAISet = Custom.GalaxyCharts.TravelerSystems.AIQuantumspaceTunnelTravelSet.Initialize()
					self.Logger.LogString(" -Initialized AI Quantumspace Tunnel Travel Set")
				except:
					print "ERROR on GetTravelSetToUse: "
					traceback.print_exc()
			pSet = myGlobalAISet #Custom.GalaxyCharts.Traveler.Travel.pAIQuantumspaceTunnelTravelSet
		return pSet
	except:
