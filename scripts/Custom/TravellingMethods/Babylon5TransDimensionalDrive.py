...
##########	MANUAL
#################################################################################################################
# NOTE: all functions/methods and attributes defined here (in this prototype example plugin, B5TransDimensionalDrive) are required to be in the plugin, with the exclusion of:
...
# ------ Auxiliar functions: "AnObjectDying", "AuxProtoElementNames", "B5TransDimensionalDriveFlash", "CreateDetachedElectricExplosion", "findShipInstance", "HasPhasedTransDimensionalDrive", "HasSigmaTransDimensionalDrive", "LoadGFX", "PlayB5TransDimensionalDriveSound", "PlayB5TransDimensionalDriveSoundC", "B5TransDimensionalDriveBasicConfigInfo" and "B5TransDimensionalDriveDisabledCalculations".
# ------ Auxiliar variables: "defineTravelSpaceNoise", "myGlobalAISet", "myGlobalpSet" and "pTransDimensionalDriveEngineSound", used for keeping tabs on our alternate pSets and the sound that plays while on jumpspace.
# ------ Auxiliar class "WhyMissionLibGetsError" and its instance "basicListener"
# ------ Auxiliar functions for intra-system intercept (ISI) support, which as a result of being a common-made function between default GalaxyCharts functions/methods, regular AlternateSubModelFTL and ISI, while not required to be on the plugin, some of their contents are actually required if they are not there: "awayNavPointDistanceCalc", "CanTravelShip", "ConditionalCloak", "ConditionalDecloak", "EngageSeqTractorCheckI", "GetEngageDirectionISI", "GetExitedTravelEventsI", "GetStartTravelEventsI", "GetEngageDirectionC", "InSystemIntercept", "MaintainTowingActionI", "PlayB5TransDimensionalDriveSoundI", "removeTractorISITowInfo", "SetupSequenceISI" and "SetupTowingI".
...
# "Core": is the name of a key whose value indicates a list of which hardpoint properties (not nacelles) are part of the B5TransDimensionalDrive system. If all are disabled/destroyed, TransDimensionalDrive will not engage either. If this field does not exist, it wil check for all subsystems with "quantum jumpspace drive" or "quantum jump-space drive", case insensitive. Use "Core": [] to skip this check.
...
# Also, important note, this mod uses additional systems for GalaxyCharts, "Custom.GalaxyCharts.TravelerSystems.TransDimensionalDriveTunnelTravelSet" and "Custom.GalaxyCharts.TravelerSystems.AITransDimensionalDriveTunnelTravelSet", with "Custom.GalaxyCharts.TravelerSystems.TransDimensionalDriveTunnelTravelSet_S" being optional but highly recommended to have.
"""
#Sample Setup: replace "EAOmega" for the appropiate abbrev. Also remove "# (#)"
Foundation.ShipDef.EAOmega.dTechs = { # (#)
	"Alternate-Warp-FTL": { # (#)
		"Setup": { # (#)
			"B5TransDimensionalDrive": {	"Nacelles": ["Proto Warp Nacelle"], "Core": ["Proto-Core"], }, # (#)
			"Body": "VasKholhr_Body",
			"NormalModel":          shipFile,
			"WarpModel":          "VasKholhr_WingUp",
			"B5TransDimensionalDriveModel":          "VasKholhr_WingUp",
			"AttackModel":          "VasKholhr_WingDown",
			"BodySetScale": 1.0,
			"NormalSetScale": 1.0,
			"WarpSetScale": 1.0,
			"B5TransDimensionalDriveSetScale": 1.0,
			"AttackSetScale": 1.0,
			"Hardpoints":       {
			...
			"B5TransDimensionalDriveHardpoints":       {
		...
		"Port Wing":     ["VasKholhr_Portwing", {
			...
			"WarpDuration":       150.0,
			"B5TransDimensionalDriveRotation":       [0, 0.749, 0],
			"B5TransDimensionalDrivePosition":       [0, 0, 0.05],
			"B5TransDimensionalDriveDuration":       150.0,
			},
		],
...
########
sName = "Sigma Walker Trans-Dimensional Drive"
...
########
sGoingTo = "Trans-dimensioning to"

########
# Phrase to show when the ship drops out of travel (while travelling)
########
sDropOut = "Dropped out of TransDimensional Jumpspace..."
...
def KindOfMove(): 
	return "B5TransDimensionalDrive" # Modify this with the name you are gonna use for the AlternateSubModelFTL
...
def IsShipEquipped(pShip):
	...
	if pInstance:
		if pInstanceDict.has_key("Alternate-Warp-FTL") and pInstanceDict["Alternate-Warp-FTL"].has_key("Setup") and pInstanceDict["Alternate-Warp-FTL"]["Setup"].has_key("B5TransDimensionalDrive"): # You need to add a foundation technology to this vessel "AlternateSubModelFTL"
			return 1
...
def HasPhasedTransDimensionalDrive(pShip, myStr = "Hyperspace Cloak"):
	found = 0
...
def HasSigmaTransDimensionalDrive(pShip):
	return HasPhasedTransDimensionalDrive(pShip, myStr = "TransDimensional Drive")
...
def AuxProtoElementNames(*args):
	# Returns hardpoint property name fragments that indicate are part of the B5TransDimensionalDrive system, and blacklists
...
def B5TransDimensionalDriveBasicConfigInfo(pShip):
	...
	if pInstance:
		...
		if pInstancedict.has_key("Alternate-Warp-FTL") and pInstancedict["Alternate-Warp-FTL"].has_key("Setup") and pInstancedict["Alternate-Warp-FTL"]["Setup"].has_key("B5TransDimensionalDrive"):
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5TransDimensionalDrive"].has_key("Nacelles"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificNacelleHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5TransDimensionalDrive"]["Nacelles"]
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5TransDimensionalDrive"].has_key("Core"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificCoreHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["B5TransDimensionalDrive"]["Core"]
...
def B5TransDimensionalDriveDisabledCalculations(type, specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pSubsystem, pShip, justFindOne=0):
	...
	return totalB5TransDimensionalDriveEngines, onlineB5TransDimensionalDriveEngines
...
def CanTravelShip(pShip):
	...
	if not isEquipped:
		...
		return "This ship is not equipped with B5TransDimensionalDrive"
			...
			if bIsPlayer == 1:
				if pHelm:
					...
					if pDatabase:
						...
						pSequence.Play ()
						App.g_kLocalizationManager.Unload (pDatabase)
			return "TransDimensionalDrive Drive is disabled"
...
def CanContinueTravelling(self):
	...
	if not isEquipped:
		if bIsPlayer == 1:
			pSequence = App.TGSequence_Create ()
			pSubtitleAction = App.SubtitleAction_CreateC("Brex: We don't have B5TransDimensionalDrive sir, we are dropping out of it.")
			pSubtitleAction.SetDuration(3.0)
			pSequence.AddAction(pSubtitleAction)
			pSequence.Play()
		bStatus = 0
		...
		if totalB5TransDimensionalDriveCores > 0 and onlineB5TransDimensionalDriveCores <= 0:
			if bIsPlayer == 1:
				pSequence = App.TGSequence_Create ()
				pSubtitleAction = App.SubtitleAction_CreateC("Brex: All TransDimensionalDrive systems are offline or disabled sir, however I've managed to get us out of TransDimensionalDrive before the systems failed utterly.")
				pSubtitleAction.SetDuration(3.0)
				pSequence.AddAction(pSubtitleAction)
				pSequence.Play()
			bStatus = 0
			...
			if totalB5TransDimensionalDriveEngines > 0 and onlineB5TransDimensionalDriveEngines <= 0:
				if bIsPlayer == 1:
					pSequence = App.TGSequence_Create ()
					pSubtitleAction = App.SubtitleAction_CreateC("Brex: All TransDimensionalDrive vortex generators are now offline or disabled sir, however I've managed to get us out of TransDimensionalDrive before the systems failed utterly")
					pSubtitleAction.SetDuration(3.0)
					pSequence.AddAction(pSubtitleAction)
					pSequence.Play()
				bStatus = 0
...
def PlayB5TransDimensionalDriveSound(pAction, pWS, sType, sRace):
	debug(__name__ + ", PlayB5TransDimensionalDriveSound")

	pShip = pWS.GetShip()
	return PlayB5TransDimensionalDriveSoundC(pAction, pShip, sType, sRace)

# Aux. ISI function
def PlayB5TransDimensionalDriveSoundI(pAction, pShipID, sType, sRace):
	debug(__name__ + ", PlayB5TransDimensionalDriveSoundI")
	pShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pShipID)
	if pShip != None:
		PlayB5TransDimensionalDriveSoundC(pAction, pShip, sType, sRace)
	return 0

# Aux. ISI function
def PlayB5TransDimensionalDriveSoundC(pAction, pShip, sType, sRace):
	debug(__name__ + ", PlayB5TransDimensionalDriveSoundC")
	...
	if pSet.GetRegionModule() == pPlaSet.GetRegionModule():
		try:
			...
			if HasSigmaTransDimensionalDrive(pShip):
				if sType == "Enter Warp":
			...
			elif HasPhasedTransDimensionalDrive(pShip):
				if sType == "Enter Warp":
			...
			try:
				...
				print "B5TransDimensionalDrive TravellingMethod: error while calling PlayB5TransDimensionalDriveSoundC:"
				traceback.print_exc()
	return 0
...
def ConditionalDecloak(pAction, pShipID, instant = 0):
	...
	if HasPhasedTransDimensionalDrive(pShip):
		if pShip.IsCloaked():
...
def CreateDetachedElectricExplosion(fRed, fGreen, fBlue, fSize, fLifeTime, sFile, pEmitFrom, pAttachTo, fFrequency=1,fEmitLife=1,fSpeed=1.0):
	...
	try:     
		...
		print "B5TransDimensionalDrive TravellingMethod: error while calling CreateDetachedElectricExplosion:"
		traceback.print_exc()
		pEffect = None
...
def B5TransDimensionalDriveFlash(pAction, pShipID, sType, sRace, sparkSize=5, sFile = 'scripts/Custom/TravellingMethods/GFX/B5TransDimensionalDriveFlashAlternate.tga'):
	pShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pShipID)
	if not pShip:
		return 0
	if pShip == None:
	...
	try:
		...
		elif HasSigmaTransDimensionalDrive(pShip):
			if sType == "Enter Warp":
				sFile = "scripts/Custom/TravellingMethods/GFX/SigmaB5TransDimensionalDriveFlash.tga"
			else:
				sFile = "scripts/Custom/TravellingMethods/GFX/SigmaB5TransDimensionalDriveFlash.tga"

		else:
			if sType == "Enter Warp":
				sFile = "scripts/Custom/TravellingMethods/GFX/B5TransDimensionalDriveFlashAlternate.tga"
			else:
				sFile = "scripts/Custom/TravellingMethods/GFX/B5TransDimensionalDriveFlash.tga"
		...
		if sparkSize > 0:
			...
			try:
				...
				print "B5TransDimensionalDrive TravellingMethod: error while calling B5TransDimensionalDriveEnterFlash:"
				traceback.print_exc()
...
def removeTractorISITowInfo(pShipID, pInstance=None):
	...
	if pInstance:
		try:
			if hasattr(pInstance, "BSTransDimensionalDriveISIvTowPosition"):
				del pInstance.BSTransDimensionalDriveISIvTowPosition
		except:
			print "BSTransDimensionalDrive: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "BSTransDimensionalDriveISITowee"):
				del pInstance.BSTransDimensionalDriveISITowee
		except:
			print "BSTransDimensionalDrive: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "BSTransDimensionalDriveISIbTractorStat"):
				del pInstance.BSTransDimensionalDriveISIbTractorStat
		except:
			print "BSTransDimensionalDrive: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
...
def SetupTowingI(ShipID):
	...
	try:
		...
		pInstance.BSTransDimensionalDriveISIvTowPosition = pTarget.GetWorldLocation()
		pInstance.BSTransDimensionalDriveISIvTowPosition.Subtract( pShip.GetWorldLocation() )
		pInstance.BSTransDimensionalDriveISIvTowPosition.MultMatrixLeft( pShip.GetWorldRotation().Transpose() )

		pInstance.BSTransDimensionalDriveISITowee = pTarget.GetObjID()

		pInstance.BSTransDimensionalDriveISIbTractorStat = 1

	except:
		print "BSTransDimensionalDrive: Error while calling SetupTowingI"
		traceback.print_exc()
...
def MaintainTowingActionI(pAction, pShipID):
	...
	if not pInstance:
		return 0
	try:
		if hasattr(pInstance, "BSTransDimensionalDriveISIbTractorStat") and pInstance.BSTransDimensionalDriveISIbTractorStat == 1:
			pTractor = pShip.GetTractorBeamSystem()
			if pTractor == None:
			...
			if not hasattr(pInstance, "BSTransDimensionalDriveISITowee"):
				return 0

			targetID = pInstance.BSTransDimensionalDriveISITowee
			...
			vPosition.Set( pInstance.BSTransDimensionalDriveISIvTowPosition )
		...
		print "BSTransDimensionalDrive: Error while calling MaintainTowingActionI"
		traceback.print_exc()
...
def EngageSeqTractorCheckI(pAction, pShipID):
	...
	if hasattr(pInstance, "BSTransDimensionalDriveISIbTractorStat") and pInstance.BSTransDimensionalDriveISIbTractorStat == 1 and hasattr(pInstance, "BSTransDimensionalDriveISITowee") and pInstance.BSTransDimensionalDriveISITowee != None:
		# hide the towee

		pToweeShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pInstance.BSTransDimensionalDriveISITowee)
		if pToweeShip:
...
myGlobalpSet = None
myGlobalAISet = None
pTransDimensionalDriveEngineSound = None
	...
	if pPlayer and hasattr(pPlayer, "GetObjID"):
		...
		if pPlayerID != None and pPlayerID != App.NULL_ID and pPlayerID == pShipID:
			try:
				...
				print "Error while shutting up the global B5 TransDimensionalDrive noise:"
				traceback.print_exc()
...
def defineTravelSpaceNoise(pAction, engage=1):
	...
	global pTransDimensionalDriveEngineSound
	if pTransDimensionalDriveEngineSound == None:
		pTransDimensionalDriveEngineSoundAux = None
		try:
			pTransDimensionalDriveEngineSoundAux = App.TGSound_Create("scripts/Custom/TravellingMethods/SFX/JumpspaceSpaceNoise.wav", "Babylon5TransDimensionalDriveStormSound", 0)
			pTransDimensionalDriveEngineSoundAux.SetSFX(0) 
			pTransDimensionalDriveEngineSoundAux.SetInterface(1)
			pTransDimensionalDriveEngineSoundAux.SetLooping(1)
		except:
			pTransDimensionalDriveEngineSoundAux = None
			print "Error on ", __name__, " defineTravelSpaceNoise:"
			traceback.print_exc()

		if pTransDimensionalDriveEngineSoundAux != None:
			pTransDimensionalDriveEngineSound = pTransDimensionalDriveEngineSoundAux
	...
	if engage == 0:
		try:
			App.g_kSoundManager.StopSound("Babylon5TransDimensionalDriveStormSound")
		...
		try:
			App.g_kSoundManager.PlaySound("Babylon5TransDimensionalDriveStormSound")
...
def SetupSequenceISI(pShip=None):
	...
	pWarpSoundAction1 = App.TGScriptAction_Create(__name__, "PlayB5TransDimensionalDriveSoundI", pShipID, "Enter Warp", sRace)
	...
	pFlashAction1 = App.TGScriptAction_Create(__name__, "B5TransDimensionalDriveFlash", pShipID, "Enter Warp", sRace)
	...
	# Create the warp flash.
	pFlashAction2 = App.TGScriptAction_Create(__name__, "B5TransDimensionalDriveFlash", pShipID, "Exit Warp", sRace)
	...
	pWarpSoundAction2 = App.TGScriptAction_Create(__name__, "PlayB5TransDimensionalDriveSoundI", pShipID, "Exit Warp", sRace)
...
def SetupSequence(self):
	...
	#	   tractor ships thru your travel. (in this example, thru TransDimensionalDrive)
	################################################################################
	...
	if (pPlayer != None) and (pShipID == pPlayer.GetObjID()):
		...
		pWarpSoundAction0 = App.TGScriptAction_Create(__name__, "PlayB5TransDimensionalDriveSoundI", pShipID, "Enter Warp", sRace)
...
def GetTravelSetToUse(self, manual=0):
	...
	try:
		...
		import Custom.GalaxyCharts.TravelerSystems.B5TransDimensionalDriveTunnelTravelSet
		import Custom.GalaxyCharts.TravelerSystems.AIB5TransDimensionalDriveTunnelTravelSet
		pSet = None
		if manual == 1:
			if myGlobalpSet == None:
				myGlobalpSet = Custom.GalaxyCharts.TravelerSystems.B5TransDimensionalDriveTunnelTravelSet.Initialize()
			if myGlobalAISet == None:
				myGlobalAISet = Custom.GalaxyCharts.TravelerSystems.AIB5TransDimensionalDriveTunnelTravelSet.Initialize()	
		if self.IsPlayer == 1:
			if myGlobalpSet == None:
				try:
					myGlobalpSet = Custom.GalaxyCharts.TravelerSystems.B5TransDimensionalDriveTunnelTravelSet.Initialize()
					self.Logger.LogString(" -Initialized B5 TransDimensionalDrive Tunnel Travel Set")
				except:
					print "ERROR on GetTravelSetToUse: "
					traceback.print_exc()
				#self.Logger.LogString(" -Initialized B5 TransDimensionalDrive Tunnel Travel Set")
			pSet = myGlobalpSet #Custom.GalaxyCharts.Traveler.Travel.pTransDimensionalDriveTunnelTravelSet
			...
			if myGlobalpSet == None:
				try:
					...
					print "ERROR on GetTravelSetToUse: "
					traceback.print_exc()
				#self.Logger.LogString(" -Initialized TransDimensionalDrive Tunnel Travel Set")
			pSet = myGlobalpSet #Custom.GalaxyCharts.Traveler.Travel.pTransDimensionalDriveTunnelTravelSet
		elif self.IsPlayer == 0 and self.IsAIinPlayerRoute == 0:
			if myGlobalAISet == None:
				try:
					myGlobalAISet = Custom.GalaxyCharts.TravelerSystems.AIB5TransDimensionalDriveTunnelTravelSet.Initialize()
					self.Logger.LogString(" -Initialized AI B5 TransDimensionalDrive Tunnel Travel Set")
				except:
					print "ERROR on GetTravelSetToUse: "
					traceback.print_exc()
			pSet = myGlobalAISet #Custom.GalaxyCharts.Traveler.Travel.pAITransDimensionalDriveTunnelTravelSet
		return pSet
	except:
...
def ConvertSpeedAmount(fSpeed):
	...
	baseSpeed = 9.999
	mediumSpeed = 5.0
	fFacA = 2.0                 ##### Do Not Change These Values #####
