...
# StarcraftWarpspace.py
# Based on the prototype custom travelling method plugin script, by USS Frontier (Enhanced Warp.py, original, template), and then modified by Alex SL Gato for StarcraftWarpspace.
# 12th September 2025
#################################################################################################################
##########	MANUAL
#################################################################################################################
# NOTE: all functions/methods and attributes defined here (in this prototype example plugin, StarcraftWarpspace) are required to be in the plugin, with the exclusion of:
...
# ------ Auxiliar functions: "AnObjectDying", "AuxProtoElementNames", "findShipInstance", "HasProtossWarp", "HasZergWarp", "LoadGFX", "PlayStarcraftWarpspaceSound", "PlayStarcraftWarpspaceSoundC", "setTheStretchyState", "StarcraftWarpspaceBasicConfigInfo", "StarcraftWarpspaceDisabledCalculations", "theStagesOfGrief" and "WatchPlayerShipLeave". TO-DO CHECK WHICH ONES ARE NO LONGER THERE
# ------ Auxiliar variables: "defineTravelSpaceNoise", "myGlobalAISet", "myGlobalpSet" and "pStarCraftWarpspaceEngineSound", used for keeping tabs on our alternate pSets and the sound that plays while on StarCraftWarpspace.
# ------ Auxiliar class "WhyMissionLibGetsError" and its instance "basicListener"
# ------ Auxiliar functions for intra-system intercept (ISI) support, which as a result of being a common-made function between default GalaxyCharts functions/methods, regular AlternateSubModelFTL and ISI, while not required to be on the plugin, some of their contents are actually required if they are not there: "awayNavPointDistanceCalc", "CanTravelShip", "ConditionalCloak", "ConditionalDecloak", "EngageSeqTractorCheckI", "GetEngageDirectionISI", "GetExitedTravelEventsI", "GetStartTravelEventsI", "GetEngageDirectionC", "InSystemIntercept", "MaintainTowingActionI", "PlayStarcraftWarpspaceSoundI", "removeTractorISITowInfo", "SetupSequenceISI" and "SetupTowingI".
...
# This main FTL method check is stored inside an "Alternate-Warp-FTL" dictionary, which is a script that should be located at scripts/Custom/Techs/AlternateSubModelFTL.py. While this sub-tech can work totally fine without such module installed, or even just act on hardpoint properties alone (including a ship subsystem that contains "warpspace drive" or "warp-space drive", case insensitive, on the hardpoint will suffice), it is recommended to have it.
# On this case, due to that, only the lines marked with "# (#)" are needed for StarcraftWarpspace to work, but the final parent technology may require more:
# "StarcraftWarpspace": is the name of the key. This is the bare minimum for the technology to work
# "Nacelles": is the name of a key whose value indicates a list of which warp engine property children (nacelles) are part of the StarcraftWarpspace system. If all are disabled/destroyed, StarcraftWarpspace will not engage. If this field does not exist or "Nacelles": [] it skips this disabled check.
# "Core": is the name of a key whose value indicates a list of which hardpoint properties (not nacelles) are part of the StarcraftWarpspace system. If all are disabled/destroyed, warpspace will not engage either. If this field does not exist, it wil check for all subsystems with "warpspace drive" or "warp-space drive", case insensitive. Use "Core": [] to skip this check.
...
Foundation.ShipDef.EAOmega.dTechs = { # (#)
	"Alternate-Warp-FTL": { # (#)
		"Setup": { # (#)
			"StarcraftWarpspace": {	"Nacelles": ["Proto Warp Nacelle"], "Core": ["Proto-Core"], }, # (#)
			"Body": "VasKholhr_Body",
			"NormalModel":          shipFile,
			"WarpModel":          "VasKholhr_WingUp",
			"StarcraftWarpspaceModel":          "VasKholhr_WingUp",
			"AttackModel":          "VasKholhr_WingDown",
			"BodySetScale": 1.0,
			"NormalSetScale": 1.0,
			"WarpSetScale": 1.0,
			"StarcraftWarpspaceSetScale": 1.0,
			"AttackSetScale": 1.0,
			"Hardpoints":       {
		...
		"Port Wing":     ["VasKholhr_Portwing", {
			...
			"WarpDuration":       150.0,
			"StarcraftWarpspaceRotation":       [0, 0.749, 0],
			"StarcraftWarpspacePosition":       [0, 0, 0.05],
			"StarcraftWarpspaceDuration":       150.0,
			},
		],
...
backwardsWarpTech = "Turret" # An aux value. Update manual if this changes
...
def KindOfMove(): 
	return "StarcraftWarpspace" # Modify this with the name you are gonna use for the AlternateSubModelFTL
...
def IsShipEquipped(pShip):
	...
	pInstance, pInstanceDict, specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist = StarcraftWarpspaceBasicConfigInfo(pShip)
	pInstance = findShipInstance(pShip)
	if pInstance:
		if pInstanceDict.has_key("Alternate-Warp-FTL") and pInstanceDict["Alternate-Warp-FTL"].has_key("Setup") and pInstanceDict["Alternate-Warp-FTL"]["Setup"].has_key("StarcraftWarpspace"): # You need to add a foundation technology to this vessel "AlternateSubModelFTL"
			return 1
	
        # For legacy reasons
        totalStarcraftWarpspaceEngines, onlineStarcraftWarpspaceEngines = StarcraftWarpspaceDisabledCalculations("Core", specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, None, pShip)
	return (totalStarcraftWarpspaceEngines > 0)

# An aux function, checks if this ship can do Protoss Warp. In order for ships that used another version can still use this, it follows similar functionality.
def HasProtossWarp(pShip, myStr = "Protoss Warp System"):
	found = 0
...
# An aux function, checks if this ship can do Zerg Warp. In order for ships that used another version can still use this, it follows similar functionality.
def HasZergWarp(pShip):
	return HasProtossWarp(pShip, myStr = "Zerg Wormhole")
...
def AuxProtoElementNames(*args):
	# Returns hardpoint property name fragments that indicate are part of the StarcraftWarpspace system, and blacklists
	return ["warpspace drive", "warp-space drive"], ["not a warpspace drive", "not a warp-space drive", " not warpspace drive", " not warp-space drive"]

# This is just another auxiliar function I made for this
def StarcraftWarpspaceBasicConfigInfo(pShip):
	...
	if pInstance:
		...
		if pInstancedict.has_key("Alternate-Warp-FTL") and pInstancedict["Alternate-Warp-FTL"].has_key("Setup") and pInstancedict["Alternate-Warp-FTL"]["Setup"].has_key("StarcraftWarpspace"):
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["StarcraftWarpspace"].has_key("Nacelles"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificNacelleHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["StarcraftWarpspace"]["Nacelles"]
			if pInstancedict["Alternate-Warp-FTL"]["Setup"]["StarcraftWarpspace"].has_key("Core"): # Use: if the tech has this field, use it. Must be a list. "[]" would mean that this field is skipped during checks.
				specificCoreHPList = pInstancedict["Alternate-Warp-FTL"]["Setup"]["StarcraftWarpspace"]["Core"]
...
# This is just another auxiliar function I made for this
def StarcraftWarpspaceDisabledCalculations(type, specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pSubsystem, pShip):
	totalStarcraftWarpspaceEngines = 0
	onlineStarcraftWarpspaceEngines = 0
	if type == "Nacelle":
		if pSubsystem != None:
				...
				if pChild:
					if hasattr(pChild, "GetName") and pChild.GetName() != None:
						...
						if found and not blacklisted:
							totalStarcraftWarpspaceEngines = totalStarcraftWarpspaceEngines + 1
							if pChild.GetCondition() > 0.0 and not pChild.IsDisabled():
								onlineStarcraftWarpspaceEngines = onlineStarcraftWarpspaceEngines + 1
	elif type == "Core":
	...
	return totalStarcraftWarpspaceEngines, onlineStarcraftWarpspaceEngines
...
def CanTravelShip(pShip):
	...
	if not isEquipped:
		...
		return "This ship is not equipped with StarcraftWarpspace"
	...
	if (specificNacelleHPList != None and len(specificNacelleHPList) > 0):
		totalStarcraftWarpspaceEngines, onlineStarcraftWarpspaceEngines = StarcraftWarpspaceDisabledCalculations("Nacelle", specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pWarpEngines, pShip)

		if totalStarcraftWarpspaceEngines <= 0 or onlineStarcraftWarpspaceEngines <= 0:
			if bIsPlayer == 1:
				if pHelm:
			...
			return "warpspace Engines disabled"

	if specificCoreHPList == None or (specificCoreHPList != None and len(specificCoreHPList) > 0):
		totalStarcraftWarpspaceCores, onlineStarcraftWarpspaceCores = StarcraftWarpspaceDisabledCalculations("Core", specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pWarpEngines, pShip)

		if totalStarcraftWarpspaceCores <= 0 or onlineStarcraftWarpspaceCores <= 0:
			if bIsPlayer == 1:
				if pHelm:
					App.CharacterAction_Create(pHelm, App.CharacterAction.AT_SAY_LINE, "CantWarp1", None, 1).Play()
				else:
					...
					if pDatabase:
						...
						pSequence.Play ()
						App.g_kLocalizationManager.Unload (pDatabase)
			return "warpspace Drive is disabled"
...
def CanContinueTravelling(self):
	...
	if not isEquipped:
		if bIsPlayer == 1:
			pSequence = App.TGSequence_Create ()
			pSubtitleAction = App.SubtitleAction_CreateC("Brex: We don't have StarcraftWarpspace sir, we are dropping out of it.")
			pSubtitleAction.SetDuration(3.0)
			pSequence.AddAction(pSubtitleAction)
			pSequence.Play()
		bStatus = 0
		...
		pInstance, pInstancedict, specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist = StarcraftWarpspaceBasicConfigInfo(pShip)
		totalStarcraftWarpspaceCores, onlineStarcraftWarpspaceCores = StarcraftWarpspaceDisabledCalculations("Core", specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pWarpEngines, pShip)
		if totalStarcraftWarpspaceCores > 0 and onlineStarcraftWarpspaceCores <= 0:
			if bIsPlayer == 1:
				pSequence = App.TGSequence_Create ()
				pSubtitleAction = App.SubtitleAction_CreateC("Brex: All warpspace systems are offline or disabled sir, however I've managed to get us out of warpspace before the systems failed utterly.")
				pSubtitleAction.SetDuration(3.0)
				pSequence.AddAction(pSubtitleAction)
				pSequence.Play()
			bStatus = 0
			...
			pWarpEngines = pShip.GetWarpEngineSubsystem()
			totalStarcraftWarpspaceEngines, onlineStarcraftWarpspaceEngines = StarcraftWarpspaceDisabledCalculations("Nacelle", specificNacelleHPList, specificCoreHPList, hardpointProtoNames, hardpointProtoBlacklist, pWarpEngines, pShip)
			if totalStarcraftWarpspaceEngines > 0 and onlineStarcraftWarpspaceEngines <= 0:
				if bIsPlayer == 1:
					pSequence = App.TGSequence_Create ()
					pSubtitleAction = App.SubtitleAction_CreateC("Brex: All warpspace vortex generators are now offline or disabled sir, however I've managed to get us out of warpspace before the systems failed utterly")
					pSubtitleAction.SetDuration(3.0)
					pSequence.AddAction(pSubtitleAction)
					pSequence.Play()
				bStatus = 0
...
def PlayStarcraftWarpspaceSound(pAction, pWS, sType, sRace):
	debug(__name__ + ", PlayStarcraftWarpspaceSound")

	pShip = pWS.GetShip()
	return PlayStarcraftWarpspaceSoundC(pAction, pShip, sType, sRace)

# Aux. ISI function
def PlayStarcraftWarpspaceSoundI(pAction, pShipID, sType, sRace):
	debug(__name__ + ", PlayStarcraftWarpspaceSoundI")
	pShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pShipID)
	if pShip != None:
		PlayStarcraftWarpspaceSoundC(pAction, pShip, sType, sRace)
	return 0

# Aux. ISI function
def PlayStarcraftWarpspaceSoundC(pAction, pShip, sType, sRace):
	debug(__name__ + ", PlayStarcraftWarpspaceSoundC")
...
def removeTractorISITowInfo(pShipID, pInstance=None):
	...
	if pInstance:
		try:
			if hasattr(pInstance, "SCIIWarpspaceISIvTowPosition"):
				del pInstance.SCIIWarpspaceISIvTowPosition
		except:
			print "SCIIWarpspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "SCIIWarpspaceISITowee"):
				del pInstance.SCIIWarpspaceISITowee
		except:
			print "SCIIWarpspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
		try:
			if hasattr(pInstance, "SCIIWarpspaceISIbTractorStat"):
				del pInstance.SCIIWarpspaceISIbTractorStat
		except:
			print "SCIIWarpspace: Error while calling removeTractorISITowInfo"
			traceback.print_exc()
...
def SetupTowingI(ShipID):
	...
	try:
		...
		pInstance.SCIIWarpspaceISIvTowPosition = pTarget.GetWorldLocation()
		pInstance.SCIIWarpspaceISIvTowPosition.Subtract( pShip.GetWorldLocation() )
		pInstance.SCIIWarpspaceISIvTowPosition.MultMatrixLeft( pShip.GetWorldRotation().Transpose() )

		pInstance.SCIIWarpspaceISITowee = pTarget.GetObjID()

		pInstance.SCIIWarpspaceISIbTractorStat = 1

	except:
		print "SCIIWarpspace: Error while calling SetupTowingI"
		traceback.print_exc()
...
def MaintainTowingActionI(pAction, pShipID):
	...
	if not pInstance:
		return 0
	try:
		if hasattr(pInstance, "SCIIWarpspaceISIbTractorStat") and pInstance.SCIIWarpspaceISIbTractorStat == 1:
			pTractor = pShip.GetTractorBeamSystem()
			if pTractor == None:
			...
			if not hasattr(pInstance, "SCIIWarpspaceISITowee"):
				return 0

			targetID = pInstance.SCIIWarpspaceISITowee
			...
			vPosition.Set( pInstance.SCIIWarpspaceISIvTowPosition )
		...
		print "SCIIWarpspace: Error while calling MaintainTowingActionI"
		traceback.print_exc()
...
def EngageSeqTractorCheckI(pAction, pShipID):
	...
	if hasattr(pInstance, "SCIIWarpspaceISIbTractorStat") and pInstance.SCIIWarpspaceISIbTractorStat == 1 and hasattr(pInstance, "SCIIWarpspaceISITowee") and pInstance.SCIIWarpspaceISITowee != None:
		# hide the towee

		pToweeShip = App.ShipClass_GetObjectByID(App.SetClass_GetNull(), pInstance.SCIIWarpspaceISITowee)
		if pToweeShip:
...
def defineTravelSpaceNoise(pAction, engage=1):
	...
	if pJumpspaceEngineSound == None:
		...
		try:
			pJumpspaceEngineSoundAux = App.TGSound_Create("scripts/Custom/TravellingMethods/SFX/JumpspaceSpaceNoise.wav", "StarcraftWarpspaceStormSound", 0)
	...
	if engage == 0:
		try:
			App.g_kSoundManager.StopSound("StarcraftWarpspaceStormSound")
		...
		try:
			App.g_kSoundManager.PlaySound("StarcraftWarpspaceStormSound")
...
def SetupSequenceISI(pShip=None):
	...
	hasTractorReady = pInstance != None and hasattr(pInstance, "SCIIWarpspaceISIbTractorStat") and (pInstance.SCIIWarpspaceISIbTractorStat == 1)
	...
	if (pPlayer != None) and (pShipID == pPlayer.GetObjID()):
		...
		if pcDest != None:
			pWatchShipLeave = App.TGScriptAction_Create(__name__, "WatchPlayerShipLeave", pcDest, pShip.GetName())
			if pWatchShipLeave != None:
				pEngageWarpSeq.AddAction(pWatchShipLeave, pCinematicStart)

	pWarpSoundAction1 = App.TGScriptAction_Create(__name__, "PlayStarcraftWarpspaceSoundI", pShipID, "Enter Warp", sRace)
	...
	pPreStretchAction = App.TGScriptAction_Create(__name__, "setTheStretchyState", pShipID, App.WarpEngineSubsystem.WES_NOT_WARPING, 1)
	pEngageWarpSeq.AddAction(pPreStretchAction, pWarpSoundAction1, 0.1)
	pStretchAction = App.TGScriptAction_Create(__name__, "setTheStretchyState", pShipID, myState, 0)
	...
	fTimeToFlash = (fEntryDelayTime*1) + (2*(App.WarpEngineSubsystem_GetWarpEffectTime()/2.0))
	fCount = 0.0
	...
	pFlashAction1 = App.TGScriptAction_Create("Actions.EffectScriptActions", "WarpFlash", pShipID)
	...
	## THIS WAS AN EXTRA ADDED TO RESTORE WARP STRETCHINESS
	#pStretchStop0Action = App.TGScriptAction_Create(__name__, "theStagesOfGrief", pShipID)
	...
	# Give it a little boost EXTRA to have a more accurate intercept
	pBoostActionR = App.TGScriptAction_Create(sCustomActionsScript, "BoostShipSpeed", pShipID, 1, -10.0)
	pExitWarpSeq.AddAction(pBoostActionR, pHideShip)

	# Check for towee
	if hasTractorReady == 1:
		pHideTowee = App.TGScriptAction_Create(sCustomActionsScript, "HideShip", pInstance.SCIIWarpspaceISITowee, 1)
		pExitWarpSeq.AddAction(pHideTowee, pHideShip)

	# Create the warp flash.
	pFlashAction2 = App.TGScriptAction_Create("Actions.EffectScriptActions", "WarpFlash", pShipID)
	pExitWarpSeq.AddAction(pFlashAction2, pHideShip, 0.7)

	# THIS WAS AN EXTRA ADDED TO RESTORE WARP STRETCHINESS
	pPreStretchEAction = App.TGScriptAction_Create(__name__, "setTheStretchyState", pShipID, App.WarpEngineSubsystem.WES_WARPING, 1)
	pExitWarpSeq.AddAction(pPreStretchEAction, pHideShip, 0.5)
	pStretchEAction = App.TGScriptAction_Create(__name__, "setTheStretchyState", pShipID, App.WarpEngineSubsystem.WES_DEWARP_INITIATED, 2)
	...
	if hasTractorReady == 1:
		pUnHideTowee = App.TGScriptAction_Create(sCustomActionsScript, "HideShip", pInstance.SCIIWarpspaceISITowee, 0)
		...
		while fCount < 3.6:
			...
			if fCount >= 3.6:
				break
		pEMaintainTowingAction = App.TGScriptAction_Create(__name__, "removeTractorISITowInfo", pShipID)
		pExitWarpSeq.AddAction(pEMaintainTowingAction, pUnHideTowee, fCount + 0.3)

	# Give it a little boost
	pBoostAction = App.TGScriptAction_Create(sCustomActionsScript, "BoostShipSpeed", pShipID, 1, 50.0)
	pExitWarpSeq.AddAction(pBoostAction, pUnHideShip, 0.1)

	# Play the vushhhhh of exiting warp
	pWarpSoundAction2 = App.TGScriptAction_Create(__name__, "PlayStarcraftWarpspaceSoundI", pShipID, "Exit Warp", sRace)
	pExitWarpSeq.AddAction(pWarpSoundAction2, pBoostAction)

	# Make the ship return to normal speed.
	pUnBoostAction = App.TGScriptAction_Create(sCustomActionsScript, "BoostShipSpeed", pShipID, 0, 1.0)
	pExitWarpSeq.AddAction(pUnBoostAction, pWarpSoundAction2, 0.5)

	## THIS WAS AN EXTRA ADDED TO RESTORE WARP STRETCHINESS
	pStretchStopE0Action = App.TGScriptAction_Create(__name__, "theStagesOfGrief", pShipID, 1)
...
def SetupSequence(self):
	...
	pBoostAction = App.TGScriptAction_Create(sCustomActionsScript, "BoostShipSpeed", pShipID, 1, 300.0)
	pEngageWarpSeq.AddAction(pBoostAction, pWarpSoundAction1, 0.7)
...
def GetStartTravelEvents(self):
	...
	return GetStartTravelEventsI(self.Ship, 1)

# Aux. ISI function
def GetStartTravelEventsI(pShip, toOtherSystem=0):
