...
class UniverseII( Carrier.Carrier):
	def __init__( self, pShip):
		...
		launcher = LauncherManager.GetLauncher( "Shuttle Hanger", pShip)
		group.AddLauncher( "Shuttle Hanger", launcher)
		launcher.AddLaunchable( "Type26Shuttle", "ftb.friendlyAI", 30)

		LauncherManager = __import__( "ftb.LauncherManager")
		launcher = LauncherManager.GetLauncher( "Fighter Bay", pShip)
		group.AddLauncher( "Fighter Bay", launcher)
		launcher.AddLaunchable( "NornFighter", "ftb.friendlyAI", 5)
...
ShipManager.RegisterShipClass( "UniverseII", UniverseII)
