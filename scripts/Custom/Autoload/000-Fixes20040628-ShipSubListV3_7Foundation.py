		...
		def __call__(self, raceShipList, menu, buttonType, uiHandler, fWidth = 0.0, fHeight = 0.0):
			...
			for race in raceList:
				...
				for key in shipList:
					...
					if type(ship.__dict__.get("SubMenu", [])) != type(ship.__dict__.get("SubMenu", [])) or type(ship.__dict__.get("SubMenu", [])) != type([]):
						continue
