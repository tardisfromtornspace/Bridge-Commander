...
# However, NormalWarp.py and EnhancedWarp.py are not NEW files, so we would need to ask USS Frontier for permission to modify the file itself.
# Unfortunately, since we cannot contact USS Frontier (according to USS Sovereign "he's gone"), we will have to get creative to fix these bugs.
# One recommended way (again, by USS Sovereign) to do so without breaking the license is via monkey patching, which would not alter the physical files and should be trivial...
...
#################################################################################################################
necessaryToUpdate = 0
filesToFix = ["NormalWarp", "EnhancedWarp"]

def FindIfNecessaryToUpdate():
	import nt
	import string
	import traceback
	global filesToFix, MODINFO, fMaximumSpeed

	dir="scripts\\Custom\\TravellingMethods" # I want to limit any vulnerability as much as I can while keeping functionality
	try:
		list = nt.listdir(dir)
		if not list:
			print "ERROR: Missing scripts/Custom/TravellingMethods folder for FIX-GCNormalWarpAndEnhancedWarp"
			return 0
	except:
		print "ERROR: Missing scripts/Custom/TravellingMethods folder for FIX-GCNormalWarpAndEnhancedWarp, or other error:"
		traceback.print_exc()
		return 0	
		...
		if (extension == "py") and not fileName == "__init__": # I am not allowing people to just use the .pyc directly, I don't want people to not include source scripts - Alex SL Gato
			...
			try:
				if not filesChecked.has_key(fileName):
					...
					myContinueS = 1
					try:
						banana = __import__(myGoodPlugin, globals(), locals(), ["MODINFO", "fMaximumSpeed", "SetupSequence"])
					except:
						print "FIX-GCNormalWarpAndEnhancedWarp: Error, could not load " + str(myGoodPlugin) + ":"
						traceback.print_exc()
						myContinueS = 0
					...
					if myContinueS == 1:
						if hasattr(banana, "MODINFO"):
							if banana.MODINFO.has_key("Version") and banana.MODINFO["Version"] >= MODINFO["Version"]:
								myContinueS = 0 # For some reason using "continue" to skip the loop here breaks the game
						if myContinueS == 1:
	...
	if needsPatching > 0:
		print "FIX-GCNormalWarpAndEnhancedWarp: Patched Stock GalaxyCharts_v2 TravellingMethods"

	return 0

FindIfNecessaryToUpdate()
