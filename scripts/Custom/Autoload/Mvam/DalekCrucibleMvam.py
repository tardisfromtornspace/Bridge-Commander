...
MvamShips = ["DalekCrucible", "DalekCrucibleBomb"]
...
IconNames = ["Detonate Reality Bomb", "Deactivate and reload$Rein"]
...
def DalekCrucibleBomb(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def. (ie: "MvamGalaxySaucer")
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "DalekCrucibleBomb")
	...
	if (pEnemies.IsNameInGroup(pShip.GetName())):
		...
		if ((pShip.GetHull().GetConditionPercentage() <= 0.95) and (pFriendlies.GetNumActiveObjects() >= (pEnemies.GetNumActiveObjects() * 2))):
			...
			if pPlayer and (pPlayer.GetHull().GetConditionPercentage() >= 0.9):
				...
				return 1
			return 1
		#small random chance of them deciding to just kaboom
		snkAiRanSep2 = App.g_kSystemWrapper.GetRandomNumber(50)
		if snkAiRanSep2 == 30:
			return 1

	#the ship is a friendly or neutral
	else:
		#seperate if we're outnumbered 7 to 1
		if ((pEnemies.GetNumActiveObjects() >= (pFriendlies.GetNumActiveObjects() * 7)) and (snkAiRanSep == 2)):
			return 1
		#seperate if we're targetting a HUGE ship (a la the Enterprise D vs. Borg). make sure the ai has a target
		try:
			if (pShip.GetTarget().GetRadius() > 20.0):
		...
		if ((pShip.GetHull().GetConditionPercentage() <= 0.5) and (pEnemies.GetNumActiveObjects() >= (pFriendlies.GetNumActiveObjects() * 2)) and (snkAiRanSep == 4)):
			return 1
		#seperate if player is at less than 10% HP
		pPlayer = MissionLib.GetPlayer()
		if pPlayer and (pPlayer.GetHull().GetConditionPercentage() <= 0.1) and (snkAiRanSep == 3):
			return 1
	return 0
