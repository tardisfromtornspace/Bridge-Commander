...
def CheckSeperate (pShip):
	...
	if (pEnemies.IsNameInGroup(pShip.GetName())):
		#seperate if we're outnumbered 10 to 1
		if ((pFriendlies.GetNumActiveObjects() >= (pEnemies.GetNumActiveObjects() * 10)) and (snkAiRanSep == 4)):
			return 1
		#seperate if 50% damaged and outnumbered 4 to 1
		if ((pShip.GetHull().GetConditionPercentage() <= 0.5) and (pFriendlies.GetNumActiveObjects() >= (pEnemies.GetNumActiveObjects() * 4)) and (snkAiRanSep == 4)):
			return 1
	#the ship is a friendly or neutral
	else:
		#seperate if we're outnumbered 10 to 1
		if (pEnemies.GetNumActiveObjects() >= (pFriendlies.GetNumActiveObjects() * 10) and (snkAiRanSep == 4)):
			return 1
		#seperate if 50% damaged and outnumbered 4 to 1
		if ((pShip.GetHull().GetConditionPercentage() <= 0.5) and (pEnemies.GetNumActiveObjects() >= (pFriendlies.GetNumActiveObjects() * 4)) and (snkAiRanSep == 4)):
			return 1
	return 0
