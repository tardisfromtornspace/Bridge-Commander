...
MvamShips = ["DiamondsSC4", "DiamondsArmoredSC4"]
...
IconNames = ["Deploy ARMOUR", "Deactivate$Rein"]
...
def DiamondsArmoredSC4(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def. (ie: "ArmoredSC4")
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "DiamondsArmoredSC4")
...
def CheckSeperate (pShip):
	#this means if the ship's hull is at or below 99%, we seperate
	if (pShip.GetHull().GetConditionPercentage() <= 99.99):
		return 1
	else:
            return 0 
...
################### for it (eg: if (pShip.GetName() == "AMVoyager"): )
...
def PreScripts(pAction, pShip):
	...
	#Added by J1NG, delete as appropriate, including this comment, as it uses space up
	#The following code is the work around for the MVAM torpedo switching problem
	#Note: That if you have more than 1 torpedo type, you have to declare them
	#Warning: DO NOT declare more torpedo types that what is in the Hard Point, other it will crash

	global pAmmount1 

	pAmmount1 = pShip.GetTorpedoSystem().GetNumAvailableTorpsToType(0)
...
def PostSeperate(pAction, pShip):
	...
	pShip.GetTorpedoSystem().LoadAmmoType(0, pAmmount1)
