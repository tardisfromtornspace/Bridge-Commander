...
MvamShips = ["BSG2003MSPlat", "BSG2003MSPlatDock"]
...
IconNames = ["Dock mode", "Mobile mode$Rein"]
...
def BSG2003MSPlatDock(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def. (ie: "MvamGalaxySaucer")
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "BSG2003MSPlatDock")
...
def CheckSeperate (pShip):
	...
	if (pEnemies.IsNameInGroup(pShip.GetName())):
		#seperate if less than 20% damaged and no enemies on sight
		if ((pShip.GetHull().GetConditionPercentage() >= 0.8) and (pFriendlies.GetNumActiveObjects() <= 0) and (snkAiRanSep == 4)):
			return 1

	#the ship is a friendly or neutral
	else:
		#seperate if less than 20% damaged and no enemies on sight
		if ((pShip.GetHull().GetConditionPercentage() >= 0.8) and (pEnemies.GetNumActiveObjects() <= 0) and (snkAiRanSep == 4)):
			return 1
	return 0
