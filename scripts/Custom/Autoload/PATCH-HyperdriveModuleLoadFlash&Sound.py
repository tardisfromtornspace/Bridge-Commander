...
# PATCH-HyperdriveModuleLoadFlash&Sound.py
# This patch was made by Alex SL Gato with permission from USS Sovereign, so as to allow flash and sound customizations from HyperdriveModule to work properly for stuff like GalaxyCharts. Hyperdrive Module remains ALL RIGHTS RESERVED, by USS Sovereign.
...
# - USS Sovereign's Hyperdrive Module, as this file's purpose revolves around it.
# 18th April 2025
# Version 0.21
...
try:
	...
	if myDependingTravelModuleAux != None and hasattr(myDependingTravelModuleAux, "VERSION") and hasattr(myDependingTravelModuleAux, "sHyperdriveList") and myDependingTravelModuleAux.VERSION < VERSION:
...
basicListener = ListenerFixClass("HyperdriveGC is listening")
basicListener.BeginListening()

def clearShipSoundInfo(pShipID):
	try:
		App.g_kSoundManager.DeleteSound("Enter Hyperdrive AI"+str(pShipID))
	except:
		traceback.print_exc()
	try:
		App.g_kSoundManager.DeleteSound("Exit Hyperdrive AI"+str(pShipID))
	except:
		traceback.print_exc()
	global shipsIDDict
...
def shipAIListenExplode(pShip, pShipID, go = "Enter"):
	try:
		App.g_kSoundManager.DeleteSound(str(go) + " Hyperdrive AI"+str(pShipID))
	except:
		traceback.print_exc()
	try:
		global shipsIDDict
		...
		def EnteringFlash(pAction, pShipID = None):
			...
			if hasattr(pModule, "HyperdriveCustomizations"):
				...
				if pCustomization.has_key('EntrySound'):
					...
					pEnterSound = App.TGSound_Create(pSound, "Enter Hyperdrive AI"+str(pShipID), 0)
					pEnterSound.SetSFX(0) 
					pEnterSound.SetInterface(1)

				# No sound entry found
				else:
					pEnterSound = App.TGSound_Create("scripts/Custom/Hyperdrive/SFX/enterslipstream.wav", "Enter Hyperdrive AI"+str(pShipID), 0)
					pEnterSound.SetSFX(0) 
					pEnterSound.SetInterface(1)
			...
			if pEnterSound != None:
				App.g_kSoundManager.PlaySound("Enter Hyperdrive AI"+str(pShipID))
				#pEnterSound.Play()
		...
		def ExitingFlash(pAction, pShipID = None):
			...
			if hasattr(pModule, "HyperdriveCustomizations"):
				...
				if pCustomization.has_key('ExitSound'):
					...
					pExitSound = App.TGSound_Create(pSound, "Exit Hyperdrive AI"+str(pShipID), 0)
					pExitSound.SetSFX(0) 
					pExitSound.SetInterface(1)

				# No sound entry found
				else:
					pExitSound = App.TGSound_Create("scripts/Custom/Hyperdrive/SFX/exitslipstream.wav", "Exit Hyperdrive AI"+str(pShipID), 0)
					pExitSound.SetSFX(0) 
					pExitSound.SetInterface(1)
			...
			if pExitSound != None:
				App.g_kSoundManager.PlaySound("Exit Hyperdrive AI"+str(pShipID))
				#pExitSound.Play()
		...
		def StartGFX(pShipID = None):
			...
			if hasattr(pModule, "HyperdriveCustomizations"):
				...
				if pCustomization.has_key('FlashAnimation'):
					...
					myDependingTravelModuleLib.LoadGFX(4, 4, GFX + "HyperdriveFlash.tga")
