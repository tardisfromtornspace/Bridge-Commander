...
# This will import the generic template and add some hull subsystems to make the tech fully compatible with a ship
# On this generic turret template you would add, for example, the important missing details from camera views (just an example)
# We would later make more specific turrets acccording to a ship, so they have similar engine techs (such as jumpspace, slipstream and such; or disabling warp engines), if those are necessary.

import App
import GlobalPropertyTemplates
	# The line below is needed by the editor to restore
	# the name of the parent script.
ParentPropertyScript = "ships.Hardpoints.GenericTurretTemplate" # TO-DO Change "ships.Hardpoints.GenericTurretTemplate" for the path of the hardpoint file to base us upon (including the name of the file).
if not ('g_bIsModelPropertyEditor' in dir(App)):
	ParentModule = __import__("ships.Hardpoints.GenericTurretTemplate", globals(), locals(), ['*']) # TO-DO Change "ships.Hardpoints.GenericTurretTemplate" for the path of the hardpoint file to base us upon (including the name of the file).
	reload(ParentModule)
...
Ship.SetShipName("GenericStarcraftIIMinotaurDominionTurret")
