...
#	It is only called by LoadBridge.Initialize("TOSBerlinBridge"), so don't
...
#	CreateBridgeModel()
#
#	Create the TOSBerlinBridge model, viewscreen and main camera, attaching them
...
def CreateBridgeModel(pBridgeSet):
	...
	pcEnvPath = "data/Models/Sets/TOSBerlinBridge/" + pcDetail[iDetail]

	# Pre-load the Bridge model and viewscreen
	App.g_kModelManager.LoadModel("data/Models/Sets/TOSBerlinBridge/TASConstitution.NIF", None, pcEnvPath)
	App.g_kModelManager.LoadModel("data/Models/Sets/TOSBerlinBridge/ConstitutionViewScreen.NIF", None, pcEnvPath)

	# Load the viewscreen and set it up specially with SetViewScreen()
	pViewScreen = App.ViewScreenObject_Create("data/Models/Sets/TOSBerlinBridge/ConstitutionViewScreen.NIF")
	pBridgeSet.SetViewScreen(pViewScreen, "viewscreen")

	# Setup bridge object
	pBridgeObject = App.BridgeObjectClass_Create("data/Models/Sets/TOSBerlinBridge/TASConstitution.NIF")
...
#	debug("Loading Galaxy red alert sound")
	...
	pGame = App.Game_GetCurrentGame()
	pGame.LoadSoundInGroup("sfx/mary-redalert-tos.wav",  "RedAlertSound", "BridgeGeneric")
