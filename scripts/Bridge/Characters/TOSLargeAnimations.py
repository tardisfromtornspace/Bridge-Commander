...
#	print("Place T")
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "TOS_stand_t_l")
	pSequence.AddAction(pAction)
	return pSequence

def TOSseatedTL(pCharacter):
#	print("Place T")
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/TOS_seatedm_t_l.nif", "TOS_seatedm_t_l")
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "TOS_seatedm_t_l")
	pSequence.AddAction(pAction)
	return pSequence
...
def TOSPlaceAtL1(pCharacter):
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/Nx_L1toG3_L.nif", "TOS_L1toG3_L")
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "TOS_L1toG3_L")
	pSequence.AddAction(pAction)
	return pSequence

	# Seat at T
def TOSSeatedT(pCharacter):
#	print("Seated T")
	kAM = App.g_kAnimationManager
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimAction_Create(pAnimNode, "TOS_seated_t_l", 0, 0, 1)
	pSequence.AddAction(pAction)
	return pSequence

def TOSTurnAtTTowardsCaptain(pCharacter):
#	print("Turn T")
	kAM = App.g_kAnimationManager
	...
	pOpenEyes = Bridge.Characters.CommonAnimations.EyesOpenMouthClosed(pCharacter)
	pSequence.AddAction(pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pCharacter.GetAnimNode(), "TOS_face_capt_t", 0, 0, 1), pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pBridgeNode, "TOS_chair_T_face_capt", 0, 0), pOpenEyes)
	return pSequence

	# Turn back T from looking at captain
def TOSTurnBackAtTFromCaptain(pCharacter):
#	print("Turn Back T")
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/TOS_face_capt_reverse.nif", "TOS_face_capt_t_reverse")
	...
	pOpenEyes = Bridge.Characters.CommonAnimations.EyesOpenMouthClosed(pCharacter)
	pSequence.AddAction(pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pCharacter.GetAnimNode(), "TOS_face_capt_t_reverse", 0, 0, 1), pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pBridgeNode, "TOS_chair_T_face_capt_reverse", 0, 0), pOpenEyes)
	return pSequence

	# lean left animation
def TOSTHit(pCharacter):
	return None
	#print("Hit T - animation doesn't exist")
	kAM = App.g_kAnimationManager
	...
	# Lean
	pLeanAction = App.TGAnimAction_Create(pAnimNode, "TOS_hit_t", 0, 0)
	pSequence.AddAction(pLeanAction)
...
#			pSequence.AddAction(App.TGSoundAction_Create("FemaleEek"+ str(App.g_kSystemWrapper.GetRandomNumber(7)+1)))
	...
	# lean hard left animation
def TOSTHitHard(pCharacter):
	return TOSTHit (pCharacter)
...
def TOSTConsoleInteraction(pCharacter):
	pCharacter = App.CharacterClass_Cast(pCharacter)
	if not (pCharacter):
		return None
