...
if int(Foundation.version[0:8]) >= 20030222:
        ...
        def EndsWithStr(sToCheck = None, sToCompare = None):
            ...
            for sCheck in sToCheck:
                lToCheck.append(sCheck)

            lToCompare = []

            for sCompare in sToCompare:
                lToCompare.append(sCompare)

            ToCheckindex = -1

            for iCheck in lToCheck:
                ToCheckindex = ToCheckindex + 1

            ToCompareindex = - 1

            for iCompare in lToCompare:
                ToCompareindex = ToCompareindex + 1
