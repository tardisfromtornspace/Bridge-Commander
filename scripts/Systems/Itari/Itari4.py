...
def Initialize():
	# Create the set ("Itari4")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Itari4")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Itari.Itari4")
	...
	# "Itari4_S.py" file with an Initialize function that creates them.
	try:
		import Itari4_S
		Itari4_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Itari4")

def Terminate():
	App.g_kSetManager.DeleteSet("Itari4")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .65, .3, .675)
	kThis.Update(0)
	kThis = None
