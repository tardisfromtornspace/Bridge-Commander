...
def Initialize():
	# Create the set ("XiEntrades2")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.XiEntrades.XiEntrades2")

	App.g_kSetManager.AddSet(pSet, "XiEntrades2")
	...
	try:
		import XiEntrades2_S
		XiEntrades2_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("XiEntrades2")

def Terminate():
	App.g_kSetManager.DeleteSet("XiEntrades2")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .5, .2, .9)
	kThis.Update(0)
	kThis = None
