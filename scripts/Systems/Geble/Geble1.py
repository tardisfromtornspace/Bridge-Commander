...
def Initialize():
	# Create the set ("Geble1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Geble1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Geble.Geble1")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Geble1")
	LoadBackdrops(pSet)
	...
	# "Geble1_S.py" file with an Initialize function that creates them.
	try:
		import Geble1_S
		Geble1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Geble1")

def Terminate():
	App.g_kSetManager.DeleteSet("Geble1")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(0)
	kThis.SetTranslateXYZ(1484.205200, 857.331726, 0.673401)
	kForward = App.TGPoint3()
