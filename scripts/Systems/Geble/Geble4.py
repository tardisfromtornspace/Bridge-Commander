...
def Initialize():
	# Create the set ("Geble4")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Geble4")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Geble.Geble4")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Geble4")
	LoadBackdrops(pSet)
	...
	# "Geble4_S.py" file with an Initialize function that creates them.
	try:
		import Geble4_S
		Geble4_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Geble4")

def Terminate():
	App.g_kSetManager.DeleteSet("Geble4")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(0)
	kThis.SetTranslateXYZ(250.205170, -4000.331787, 0.673401)
	kForward = App.TGPoint3()
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(0)
	kThis.SetTranslateXYZ(450.637909, -4000.833008, 0.343842)
	kForward = App.TGPoint3()
...
def LoadBackdrops(pSet):
	...
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.049818, 0.254100, 0.965894)
	kThis.AlignToVectors(kForward, kUp)
