...
def Initialize():
	# Create the set ("Albirea1")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Albirea.Albirea1")

	App.g_kSetManager.AddSet(pSet, "Albirea1")
	...
	# "Albirea1_S.py" file with an Initialize function that creates them.
	try:
		import Albirea1_S
		Albirea1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Albirea1")

def Terminate():
	App.g_kSetManager.DeleteSet("Albirea1")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(0)
	kThis.SetTranslateXYZ(200.490982, 400.286932, -300.749420)
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .6, .6, .75)
	kThis.Update(0)
	kThis = None
