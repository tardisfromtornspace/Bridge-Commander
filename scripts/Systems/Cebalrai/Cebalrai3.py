...
def Initialize():
	# Create the set ("Cebalrai3")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Cebalrai3")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cebalrai.Cebalrai3")
	...
	# "Cebalrai3_S.py" file with an Initialize function that creates them.
	try:
		import Cebalrai3_S
		Cebalrai3_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Cebalrai3")

def Terminate():
	App.g_kSetManager.DeleteSet("Cebalrai3")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .9, .9, .3)
	kThis.Update(0)
	kThis = None
