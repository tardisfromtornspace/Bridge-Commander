...
def Initialize():
	# Create the set ("Cebalrai2")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Cebalrai2")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cebalrai.Cebalrai2")
	...
	# "Cebalrai2_S.py" file with an Initialize function that creates them.
	try:
		import Cebalrai2_S
		Cebalrai2_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Cebalrai2")

def Terminate():
	App.g_kSetManager.DeleteSet("Cebalrai2")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .7, .7, .6)
	kThis.Update(0)
	kThis = None
	...
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.231592, 0.968491, 0.091597)
	kUp = App.TGPoint3()
