...
def Initialize():
	# Create the set ("Alioth8")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Alioth.Alioth8")

	App.g_kSetManager.AddSet(pSet, "Alioth8")
	...
	# "Alioth8_S.py" file with an Initialize function that creates them.
	try:
		import Alioth8_S
		Alioth8_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Alioth8")

def Terminate():
	App.g_kSetManager.DeleteSet("Alioth8")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(0)
	kThis.SetTranslateXYZ(-100.00, 1000.00, 0.00)
	kForward = App.TGPoint3()
