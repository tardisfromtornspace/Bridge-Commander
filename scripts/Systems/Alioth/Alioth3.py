...
def Initialize():
	# Create the set ("Alioth3")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Alioth.Alioth3")

	App.g_kSetManager.AddSet(pSet, "Alioth3")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Alioth3")
	LoadBackdrops(pSet)
	...
	# "Alioth3_S.py" file with an Initialize function that creates them.
	try:
		import Alioth3_S
		Alioth3_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Alioth3")

def Terminate():
	App.g_kSetManager.DeleteSet("Alioth3")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, 1, .65, .675)
	kThis.Update(0)
	kThis = None
