...
def Initialize():
	# Create the set ("Voltair1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Voltair1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Voltair.Voltair1")
	...
	# "Voltair1_S.py" file with an Initialize function that creates them.
	try:
		import Voltair1_S
		Voltair1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Voltair1")

def Terminate():
	App.g_kSetManager.DeleteSet("Voltair1")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .75, .75, .525)
	kThis.Update(0)
	kThis = None
