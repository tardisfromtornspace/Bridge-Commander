...
def Initialize():
	# Create the set ("Yiles3")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Yiles3")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Yiles.Yiles3")
	...
	# "Yiles3_S.py" file with an Initialize function that creates them.
	try:
		import Yiles3_S
		Yiles3_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Yiles3")

def Terminate():
	App.g_kSetManager.DeleteSet("Yiles3")
...
def LoadPlacements(sSetName):
	...
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .8, .65, .45)
	kThis.Update(0)
	kThis = None
