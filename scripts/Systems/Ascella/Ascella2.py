...
def Initialize():
	# Create the set ("Ascella2")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Ascella.Ascella2")

	App.g_kSetManager.AddSet(pSet, "Ascella2")
	...
	# "Ascella2_S.py" file with an Initialize function that creates them.
	try:
		import Ascella2_S
		Ascella2_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Ascella2")

def Terminate():
	App.g_kSetManager.DeleteSet("Ascella2")

def LoadPlacements(sSetName):
	# Position "Planet1"
	kThis = App.Waypoint_Create("Planet1", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-40.886318, 1000.501465, 0.000000)
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .55, .275, .825)
	kThis.Update(0)
	kThis = None
