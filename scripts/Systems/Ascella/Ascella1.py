...
def Initialize():
	# Create the set ("Ascella1")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Ascella.Ascella1")

	App.g_kSetManager.AddSet(pSet, "Ascella1")
	...
	# "Ascella1_S.py" file with an Initialize function that creates them.
	try:
		import Ascella1_S
		Ascella1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Ascella1")

def Terminate():
	App.g_kSetManager.DeleteSet("Ascella1")

def LoadPlacements(sSetName):
	# Position "Planet1"
	kThis = App.Waypoint_Create("Planet1", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-140.886318, 300.501465, 0.000000)
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .4, .1, 1)
	kThis.Update(0)
	kThis = None
